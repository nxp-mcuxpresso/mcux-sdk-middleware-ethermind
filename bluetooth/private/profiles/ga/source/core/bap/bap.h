...
#define BAP_CHAR_ID_SINK_PAC_COUNT                      GA_CONFIG_LIMITS(GA_MAX_SINK_PAC_CONTEXTS)
...
#define BAP_CHAR_ID_SOURCE_PAC_COUNT                    GA_CONFIG_LIMITS(GA_MAX_SOURCE_PAC_CONTEXTS)
...
/** PAC Record */
typedef struct _BAP_PAC_RECORD_DB
{
    /* PACS Char Instance for this record */
    GA_BRR_CHR_INST bap_pacs_inst;

    /* Number of PAC records in a single Char Instance */
    UINT8 bap_num_pac;

    BAP_PAC_RECORD bap_pac_rec[GA_MAX_PAC_RECORDS_IN_SINGLE_INST];
...
typedef struct _BAP_CAPABILITY_INFO
{
    void   * data;
    UINT16   datalen;
    UINT8    chr_inst;
...
GA_RESULT BAP_broadcast_update_sep
          (
              /* IN */ UINT8 sep,
              /* IN */ UINT8 ssn,
              /* IN */ GA_CODEC_INFO * codec,
...
GA_RESULT BAP_get_context_record_count
          (
              BAP_HANDLE handle,
              UINT8 * src_pac_count,
              UINT8 * snk_pac_count,
