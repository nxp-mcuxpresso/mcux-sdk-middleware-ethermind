...
#define PBP_DECODE_BCAST_ANNOUNCEMENT_FEATURES(features, en, sq, hq) \
        (en) = (features); \
        (en) = ((PBP_BCAST_ENCRYPTION_ON << PBP_BCAST_ENCRYPTION_FEATURE_BIT) & en); \
        (sq) = (features); \
        (sq) = ((PBP_BCAST_SQ_ON << PBP_BCAST_SQ_FEATURE_BIT) & sq); \
        (hq) = (features); \
        (hq) = ((PBP_BCAST_HQ_ON << PBP_BCAST_HQ_FEATURE_BIT) & hq);
