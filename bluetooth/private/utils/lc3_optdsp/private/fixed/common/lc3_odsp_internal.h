...
#ifndef _H_LC3_ODSP_INTERNAL_
#define _H_LC3_ODSP_INTERNAL_

/* --------------------------------------------- Header File Inclusion */
#include "EM_os.h"
#include "lc3_odsp_defines.h"
#include "LC3_odsp_features.h"
...
typedef struct
{
    ...
    INT16         nbits_est;
    INT16         x_exp;
    __ALIGNED(4)  INT16         mdct_mem[LC3_MAX_IDCT_MEM_SIZE];				//the size of these buffer is sampling rate dependent.  If 48k is always supported, it can a buffer directly 
    ...
    //__ALIGNED(4)	INT16         pd_mem_12_8[3];
    INT16         pd_mem_12_8[3];
...
typedef struct
{
    ...
    INT16         pd_mem_x_exp;
    __ALIGNED(4)  INT16        ltpf_mem_x[LC3_LTPF_X_LEN];		   //the size of these buffer is sampling rate dependent.  We may define the size of the array based on the highest fs supported
...
void lc3_odsp_cmn_pvq_normalize_vector ( INT16 *xq, const INT16 *y,const INT32  k_val_max );

void lc3_odsp_cmn_sns_scalefactor_interpolation
     (
                INT16 *scf_q,
                INT16 mdct_scf[],
                INT16 mdct_scf_exp[],
...
void lc3_odsp_cmn_sns_spectral_shaping
     (
         INT32 x[],
         INT16 scf[],
         INT16 scf_exp[],
...
void lc3_odsp_cmn_dct_ii_16( INT16 *in, INT16 *out);

void lc3_odsp_cmn_idct_ii_16( INT16 *in, INT16 *out);

void lc3_odsp_cmn_dct_iv( INT32 *pDat,INT16 *pDat_e, INT32  len, INT32 *work_buffer);

INT32 lc3_odsp_cmn_get_bw_cutoff_max(INT32 idx);

void lc3_odsp_init_set_mdct_params(INT32 n_dms, INT32 f_s_ind,const INT16 **w, INT16 *w_len, INT16 *z_len);

INT16 lc3_odsp_math_get_one_by_sqrt(INT16 x, INT16 *x_e);

INT16 lc3_odsp_math_get_one_by_num(INT16 x, INT16 *x_e);

INT16 lc3_odsp_cmn_get_norm_scale_16bit(INT16 *x, INT16 len);

INT16 lc3_odsp_cmn_get_norm_scale_l_0 (INT32 *x, INT16 len);

INT16 lc3_odsp_cmn_get_fx_division(INT32 x, INT32 y, INT32 *scale);

INT16 lc3_odsp_cmn_get_fx_division_s(INT32 x, INT32 y, INT32 *scale);

INT32 lc3_odsp_math_pow2(INT32 x);

INT32 lc3_odsp_cmn_dot_product(INT32 *x, INT32 scale, INT32 len, INT32 *y_e);

void lc3_odsp_math_scaling(INT16 *x, INT32 len, INT32 e);

INT32 lc3_odsp_math_get_fx_add(INT32 x1, INT32 x1_e, INT32 x2, INT32 x2_e, INT32 *y_e);

INT16 lc3_odsp_math_get_fx_compare(INT32 x1, INT32 x1_e, INT32 x2, INT32 x2_e);

INT32 lc3_odsp_math_get_one_by_sqrt_l (INT32 x, INT32 *x_e);

void lc3_odsp_fft_complex_fft(INT32 *real, INT32 *imag, INT32 length, INT16 *scale, INT32 *x);

#ifdef __cplusplus
};
#endif

#endif /* _H_LC3_ODSP_INTERNAL_ */
