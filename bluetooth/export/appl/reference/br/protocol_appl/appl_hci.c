...
/* TODO: Clean up with HCI interface to get address from handle */
UCHAR g_acl_bd_addr[BT_BD_ADDR_SIZE];
           ...
           )
{
    ...
    case HCI_INQUIRY_COMPLETE_EVENT:
        LOG_DEBUG("Received HCI_INQUIRY_COMPLETE_EVENT.\n");

        /* Status */
        hci_unpack_1_byte_param(&status, event_data);
        LOG_DEBUG("\tStatus: 0x%02X\n", status);

        break;

    case HCI_INQUIRY_RESULT_EVENT:
        LOG_DEBUG("Received HCI_INQUIRY_RESULT_EVENT.\n");

        /* Number of Responses */
        hci_unpack_1_byte_param(&num_responses, event_data);
        LOG_DEBUG("\tNum Response: %d (0x%02X)\n", num_responses, num_responses);
        event_data += 1U;
        ...
        for (count = 0U; count < num_responses; count ++)
        {
            ...
            LOG_DEBUG("\tPage Scan Repetition Mode: 0x%02X\n", value_1);
            event_data += 1U;
    ...
    case HCI_CONNECTION_COMPLETE_EVENT:
        ...
        BT_COPY_BD_ADDR(g_acl_bd_addr, bd_addr);
    ...
    case HCI_ENCRYPTION_CHANGE_EVENT:
        ...
        case 0x02U:
...
#ifdef BT_GAM
            {
                ...
                retval = BT_sm_get_device_link_key(g_acl_bd_addr, link_key);

                if (API_SUCCESS == retval)
                {
                    BT_COPY_BD_ADDR(bdaddr.addr, g_acl_bd_addr);
                    bdaddr.type = 0U;
                    appl_ga_bonded_connection(&bdaddr, link_key);
                }
            }
#endif /* BT_GAM */
            break;
...
#define printf PRINTF
    ...
    case HCI_SIMPLE_PAIRING_COMPLETE_EVENT:
        CONSOLE_OUT("Received HCI_SIMPLE_PAIRING_COMPLETE_EVENT.\n");
    ...
    case HCI_LINK_SUPERVISION_TIMEOUT_CHANGED_EVENT:
        CONSOLE_OUT("Received HCI_LINK_SUPERVISION_TIMEOUT_CHANGED_EVENT.\n");
        ...
        hci_unpack_2_byte_param(&value_2, event_data);
        CONSOLE_OUT("\tLink Supervision Timeout: 0x%04X\n", value_2);
...
#endif /* BT_LE */
        {
            LOG_DEBUG("Unknown Event Code 0x%02X Received.\n", event_type);
        }
        break;
    }
...
#endif /* BT_HCI_1_2 */
#else /* BR_EDR_HCI */
            CONSOLE_OUT("BR_EDR_HCI flag is not defined!\n");
#endif /* BR_EDR_HCI */
            break;
...
void appl_hci_write_extended_inquiry_response(void)
{
    ...
    /* Set Local Name */
    eir_data[eir_num_sets].eir_data_type = HCI_EIR_DATA_TYPE_COMPLETE_LOCAL_NAME;
    eir_data[eir_num_sets].eir_data = device_name;
    eir_data[eir_num_sets].eir_data_len = (UCHAR)BT_str_len(device_name);
    eir_num_sets++;

#ifdef BT_GAM
    appl_ga_utils_get_cs_rsi(rsi, &rsilen);

    eir_data[eir_num_sets].eir_data_type = HCI_EIR_DATA_TYPE_RSI;
    eir_data[eir_num_sets].eir_data = rsi;
    eir_data[eir_num_sets].eir_data_len = rsilen;
    eir_num_sets++;
#endif /* BT_GAM */
    ...
    eir_data[eir_num_sets].eir_data_type = HCI_EIR_DATA_TYPE_COMPLETE_16_BIT_UUIDS;
    eir_data[eir_num_sets].eir_data = uuids;
    eir_data[eir_num_sets].eir_data_len = (UCHAR)num_uuids;
    eir_num_sets++;
#endif /* SDP_SERVER */
...
void appl_vendor_specific_commands (void)
{
    ...
    printf ("%s", appl_vendor_specific_menu);
    ...
    BT_LOOP_FOREVER()
    {
        ...
        case 1:
            printf ("%s", appl_vendor_specific_commands);
            break;
