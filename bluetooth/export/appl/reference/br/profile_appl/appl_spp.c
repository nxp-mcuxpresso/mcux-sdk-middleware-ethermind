            ...
            if (0U == spp_num_instances_to_send_data)
            {
                ...
                for (i = 0U; i < SPP_MAX_ENTITY; i++)
                {
                        ...
                        (SPP_CONNECTED == appl_spp_conn_status_list[i].status))
                    {
                        ...
                        if (read < 0)
                        {
                            appl_spp_conn_status_list[i].appl_nbytes = 0;
                            printf("Invalid no. of bytes to write \n");
                        }
                        else
                        {
                            ...
                            printf("Loop  = "); fflush(stdout);
                            scanf("%d", &read);
                            appl_spp_conn_status_list[i].appl_loop = ((read < 0) ? (UINT16)1U : (UINT16)read);
#endif /* NXP_CODE */
                        }
                    }
                }
                ...
                for (i = 0U; i < SPP_MAX_ENTITY; i++)
                {
                    if ((SPP_HANDLE_INVALID != appl_spp_conn_status_list[i].spp_handle) &&
                        (SPP_CONNECTED == appl_spp_conn_status_list[i].status))
                    {
                        if (appl_spp_conn_status_list[i].appl_nbytes > 0U)
                        {
           ...
           )
{
    ...
    case SPP_SEND_CNF:
    {
        ...
        else
        {
            ...
            for (count = 2U; count < appl_spp_conn_status_list[spp_handle].appl_nbytes; count ++)
            {
                buffer[count] = (UCHAR)(count - 2U);
            }
        }
