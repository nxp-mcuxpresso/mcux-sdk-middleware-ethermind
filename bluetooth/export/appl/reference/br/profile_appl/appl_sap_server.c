...
-------- SIM Access Server Menu ------- \n\
    ...
    3.  Generate PIN \n\
    4.  Set SAP Server Class of Device \n\
 \n\
   10.  Report Status \n\
 \n\
...
void main_sap_server_operations(void)
{
    ...
    BT_LOOP_FOREVER()
    {
        ...
        case 3: /* Generate PIN */
            CONSOLE_OUT("Enter the length of the PIN\n");
            CONSOLE_IN("%d", &choice);
            pin_len = (UCHAR)choice;
            sap_server_appl_passwd_generator(passwd, pin_len);
            passwd[pin_len] = '\0';
            CONSOLE_OUT("Generated Pass Key: %s\n", passwd);
            break;
