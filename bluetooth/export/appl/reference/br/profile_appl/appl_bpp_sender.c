...
\t31. Get RUI\n\
\n \
\t50. Enter Server Pincode for Connection\n\
...
#ifdef APPL_BPP_SENDER_SDP_EXT
    UCHAR bpp_char_repertoires_attr_id_val[16U];
    UINT16 bpp_char_repertoires_attr_id_size;

    UINT16 attrib_id[4U] = { PROTOCOL_DESC_LIST,
        ADDITIONAL_PROT_DESC_LIST_ID,
        GOEP_L2CAP_PSM_ATTR_ID,
        BPP_CHARACTER_REPERTOIRES_SUPPORTED_ATTR_ID};
        ...
        LOG_DEBUG("\n");
        {
            ...
            while (NULL != sdp_query_result)
            {
                ...
                if (API_SUCCESS == retval)
                {
                    ...
                    BT_mem_set(bpp_char_repertoires_attr_id_val, 0, sizeof(bpp_char_repertoires_attr_id_val));
                    bpp_char_repertoires_attr_id_size = 16U;
                    ...
                    retval = BT_sdp_get_attr_value
                             (
                                 BPP_CHARACTER_REPERTOIRES_SUPPORTED_ATTR_ID,
                                 sdp_query_result,
                                 bpp_char_repertoires_attr_id_val,
                                 &bpp_char_repertoires_attr_id_size
                             );

                    if (API_SUCCESS != retval)
                    {
                        LOG_DEBUG("SDP FAILED to find BPP_CHARACTER_REPERTOIRES_SUPPORTED_ATTR_ID\n");
                    }
                    else
                    {
                        LOG_DEBUG("BPP_CHARACTER_REPERTOIRES_SUPPORTED_ATTR_ID:0x");
                        for (i = 0U; i < bpp_char_repertoires_attr_id_size; i++)
                        {
                            LOG_DEBUG("%02X", bpp_char_repertoires_attr_id_val[i]);
                        }
                        LOG_DEBUG("\n");
                    }
                }
