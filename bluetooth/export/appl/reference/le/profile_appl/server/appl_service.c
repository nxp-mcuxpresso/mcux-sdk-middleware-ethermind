...
UCHAR appl_manufacturer_name_ext[] = APPL_MANUFACTURER_NAME;
...
#ifdef APPL_NEW_THREAD_POST_MSG
OSA_MSGQ_HANDLE_DEFINE(appl_post_queue, 10, sizeof(void*));
#endif /* APPL_NEW_THREAD_POST_MSG */
...
#ifdef APPL_NEW_THREAD_POST_MSG
TaskHandle_t appl_service_post_task_handler = NULL;

void appl_service_post_msg(void * handle)
{
    APPL_HANDLE temp = *(APPL_HANDLE *) handle;
    (void)OSA_MsgQPut(appl_post_queue, (void *)&temp);
}

void appl_service_post_proc(void *param)
{
    APPL_HANDLE appl_handle;

    OSA_MsgQCreate(appl_post_queue, 10, sizeof(void *));

    while(1)
    {
        if (KOSA_StatusSuccess == OSA_MsgQGet(appl_post_queue, &appl_handle, osaWaitForever_c))
        {
            APPL_SEND_MEASUREMENT(&appl_handle);
        }
    }
}
#endif /* APPL_NEW_THREAD_POST_MSG */
...
{
#ifdef APPL_NEW_THREAD_POST_MSG
    /* When using the IMU interface to receive data, an Event will be sent in the IMU interrupt to wake up
     * the IMU_Receive task. This process requires the assistance of the TIM_SVC task.
     * When CPU3 wakes up CPU2, the EtherMind WT task will be suspended, and IMU_Receive is needed to resume it.
     * TIM_SVC and EtherMind WT share a mutex, so TIM_SVC is also blocked because it cannot get the mutex (occupied by EtherMind WT).
     * Therefore, IMU_Receive, EtherMind WT and TIM_SVC formed a deadlock.
     * So need to create a new task needs to avoid this. */

    if (appl_service_post_task_handler == NULL)
    {
        xTaskCreate(appl_service_post_proc, "APPL_PostProc", BT_TASK_STACK_DEPTH, NULL, BT_TASK_PRIORITY, &appl_service_post_task_handler);
    }
#endif /* APPL_NEW_THREAD_POST_MSG */
...
{
    /* Proceed only when the GAP Auto Procedure state is True */
...
*         Target Address AD Type to be added from this offset.
*
*  \note:
...
UCHAR appl_get_gatt_server_authorization_state(DEVICE_HANDLE dq_handle)
{
     ...
     * TODO: Have Authorization based on Peer Device Instance.
     */
