 ...
 * ensure that all the associated Bonded devices retrieved from persistent
 ...
 * resolving list. Any ADV packet with the added Peer Identity address is not
...
/* Application SMP pair-able mode */
DECL_STATIC UCHAR pairable_mode = SMP_TRUE;

/* Application SMP MITM Requirement [TRUE or FALSE] */
DECL_STATIC UCHAR g_appl_mitm_req = SMP_FALSE;

#ifdef SMP_LESC
/* Application SMP Pairing Mode [Legacy or LESC] */
...
============= SMP PL MENU ==============\n\
 ...
 43. Set Platform MITM Requirement \n\
 \n\
...
void appl_smp_lesc_xtxp_ltk_complete(SMP_LESC_LK_LTK_GEN_PL * xtxp)
{
    ...
    CONSOLE_OUT("Adding Device to Device SMP DB .. %02X:%02X:%02X:%02X:%02X:%02X - 0x%02X\n",
...
void appl_smp_lesc_xtxp_lk_complete(SMP_LESC_LK_LTK_GEN_PL * xtxp)
{
    ...
    CONSOLE_OUT("Adding Device to Device SM DB .. %02X:%02X:%02X:%02X:%02X:%02X\n",
    appl_smp_bd_addr.addr[0], appl_smp_bd_addr.addr[1], appl_smp_bd_addr.addr[2],
    appl_smp_bd_addr.addr[3], appl_smp_bd_addr.addr[4], appl_smp_bd_addr.addr[5]);
                       ...
                       )
{
    ...
    for (index = data_length - 1U; index >= 0; index -= 2)
    {
        if (0 != index)
        {
            c1 = data[index];
    ...
    BT_LOOP_FOREVER()
    {
        ...
        switch(choice)
        {
            ...
            case 3:
                ...
                APPL_TRC("(0-MITM Best Effort, 1-MITM Strict): ");
                CONSOLE_IN("%u", &read_val);
            ...
            case 4:
                ...
                APPL_TRC("(0-LESC Best Effort, 1-LESC Strict): ");
                CONSOLE_IN("%u", &read_val);
            ...
            case 43:
                {
                    CONSOLE_OUT("\nEnter the desired MITM requirement for Platform : ");
                    CONSOLE_OUT("\n 0 - No MITM Required ");
                    CONSOLE_OUT("\n 1 - MITM is Required");
                    CONSOLE_OUT("\nYour Choice?");
                    CONSOLE_IN("%u", &read_val);

                    g_appl_mitm_req = (0x01U == (UCHAR) read_val) ? SMP_TRUE : SMP_FALSE;
                }
                break;
    ...
    BT_LOOP_FOREVER()
    {
        ...
        switch(choice)
        {
            ...
            case 3:
                ...
                /* Call SMP LE Secure Connections Mode Setting API here */
                auth.pair_mode = (UCHAR)((read_val)?SMP_LESC_MODE:SMP_LEGACY_MODE);
                ...
                /**
                 * Update the Default MITM mode related global to the value
                 * selected by User.
                 */
                g_appl_mitm_req = (SMP_SEC_LEVEL_2 == auth.security) ?
                                  SMP_TRUE : SMP_FALSE;
            ...
            case 23:
                ...
                {
                    CONSOLE_OUT("Use Option 21 to set Automatic mode\n");
                }
                break;
            ...
            case 61:
#ifdef SMP_DATA_SIGNING
                {
                    ...
                    if (NULL != data)
                    {
                        ...
                        /* Call to sign the data */
                        appl_smp_verify_sign_data(bd_handle, data, datalen);
            ...
            case 101:
#ifdef SMP_LESC_CROSS_TXP_KEY_GEN
                {
                    ...
                    /* Get the BD handle */
                    retval = BT_smp_get_link_handle(&bd_addr, &bd_handle, SMP_LINK_BREDR);
...
void appl_smp_verify_sign_data (DEVICE_HANDLE handle, UCHAR * data, UINT16 datalen)
{
    API_RESULT retval;
    ...
    if (API_SUCCESS != retval)
    {
        APPL_TRC("\n*** Device not in SMP Bonded Database to verify signing. Discarding... ***\n\n");
        return;
    }
           ...
           )
{
    ...
    case SMP_AUTHENTICATION_COMPLETE:
...
#endif /* SMP_LESC_CROSS_TXP_KEY_GEN */
        {
            ...
            if (API_SUCCESS == retval)
            {
                appl_ga_bonded_connection(&bdaddr, p_key_info.enc_info);
            }
#endif /* BT_GAM */
#endif /* APPL_FSM_SUPPORT */
        }
    ...
    case SMP_AUTHENTICATION_REQUEST:
        ...
        /**
         * Choose the MITM in Response based on what is configured by the
         * application.
         * if MITM Req is set to True, then set the Security Level to 2
         */
        if (SMP_TRUE == g_appl_mitm_req)
        {
...
#ifdef BT_GAM
        ga_rpa_complete(event, status, eventdata, data_len);
#endif /* BT_GAM */
        break;
