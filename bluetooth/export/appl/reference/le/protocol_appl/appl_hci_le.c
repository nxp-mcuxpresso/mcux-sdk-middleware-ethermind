...
#define CONN_ISO_INTERVAL_US 1250
...
#endif /* BT_5_2 */
    /*VSC*/
    /*Opcode added here to make sure its found in LE only scenario */
#ifdef ENABLE_BT_IND_RESET
    { 0xFC0DU, "HCI_VENDOR_SET_IR_CONFIG_OPCODE" },
#endif
};
...
#ifdef ENABLE_BT_IND_RESET
            else if (HCI_VENDOR_SET_IR_CONFIG_OPCODE == value_2)
            {
                g_ir_hci_cmd_status = status;
            }
#endif
...
#define APPL_TRC printf
        ...
        default:
            LOG_DEBUG("LE: Unknown Event Code 0x%02X Received.\n", event_type);
            break;
        }
    }
...
#endif /* APPL_LIMIT_LOGS */
        LOG_DEBUG("Device Address: " BT_DEVICE_ADDR_SPACED_FRMT_SPECIFIER "\n",
        BT_DEVICE_ADDR_PRINT_STR(&APPL_HCI_LE_GET_PEER_ADV_ADDR(index)));

#ifndef APPL_LIMIT_LOGS
        LOG_DEBUG("RSSI          : 0x%02X\n", APPL_HCI_LE_GET_PEER_ADV_RSSI(index));
        LOG_DEBUG("ADV Len       : 0x%02X\n", APPL_HCI_LE_GET_PEER_ADV_DATALEN(index));
        LOG_DEBUG("ADV Data      : \n");
        appl_dump_bytes
        (
            APPL_HCI_LE_GET_PEER_ADV_DATA(index),
