...
/**
 * This Flag enable the detail prints for LEPC.
 */
#define APPL_HCI_LE_LEPC_DBGLOG
...
#ifdef NXP_CODE
uint8_t local_cis_bis_feature_set;
/*
typedef struct local_features_bitmap_s
{
	uint8_t LE_encryption :1;
	uint8_t Connection_parameters_request_procedure :1;
	uint8_t Extended_reject_indication :1;
	uint8_t Peripheral_initiated_features_exchange :1;
	uint8_t LE_ping :1;
	uint8_t LE_data_packet_length_extension :1;
	uint8_t LL_privacy :1;
	uint8_t Extended_scanner_filter_policies :1;
	uint8_t LE_2M_phy :1;
	uint8_t Stable_modulation_index_tx :1;
	uint8_t Stable_modulation_index_rx :1;
	...
	uint8_t Minimum_number_of_used_channels :1;
	...
	uint8_t Connected_Isochronous_Stream_Central:1;
	uint8_t Connected_Isochronous_Stream_Peripheral :1;
	uint8_t Isochronous_Broadcaster: 1;
    uint8_t Synchronized_Receiver: 1;
    uint8_t Connected_Isochronous_Stream_host :1;
    ...
    uint8_t Connection_subrating_host :1;
    uint8_t Channel_classification :1;
    uint16_t reserved;
    uint8_t reserved1 :7;
}local_features_bitmap_t;

local_features_bitmap_t local_feature;
*/
#endif
...
#ifdef APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV
/** Global to have Auto initiate LE connection on selected ADV Report Reception */
DECL_STATIC UCHAR appl_hci_le_auto_conn_state = BT_TRUE;
#endif /* APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV */

#ifdef NXP_CODE
tx_power Ble_tx_power;
UCHAR version_info_array[9] ={0};
#endif /* NXP_CODE */
...
void appl_print_fw_rom_version(UCHAR event_datalen)
{
    LOG_DEBUG(" \n=================================================================\n");
    if (event_datalen >= 12)
    {
        if (event_datalen >= 13)
        {
            LOG_DEBUG("\tController Firmware_Version  - %02X.%02X.%02X.p%02X.%02X / %d.%d.%d.p%d.%d\n",
                      version_info_array[2U], version_info_array[1U], version_info_array[0U], version_info_array[3U],
                      version_info_array[8U], version_info_array[2U], version_info_array[1U], version_info_array[0U],
                      version_info_array[3U], version_info_array[8U]);
        }
        else
        {
            LOG_DEBUG("\tController Firmware_Version  - %02X.%02X.%02X.p%02X / %d.%d.%d.p%d\n", version_info_array[2U],
                      version_info_array[1U], version_info_array[0U], version_info_array[3U], version_info_array[2U],
                      version_info_array[1U], version_info_array[0U], version_info_array[3U]);
        }
        LOG_DEBUG("\tController ROM_Version  - %02X.%02X.%02X.%02X / %d.%d.%d.%d\n", version_info_array[4U],
                  version_info_array[5U], version_info_array[6U], version_info_array[7U], version_info_array[4U],
                  version_info_array[5U], version_info_array[6U], version_info_array[7U]);
    }
    else
    {
        LOG_DEBUG("\tError while reading FW version\n");
    }
    LOG_DEBUG(" \n=================================================================\n");
}
#endif /* NXP_CODE */
           ...
           )
{
    ...
    if (HCI_NUMBER_OF_COMPLETED_PACKETS_EVENT == event_type)
    {
        ...
        UINT16 num_of_completed_pkts;
        static int src_sent_ms=0;
        int now_ms = 1;
...
/*        printf("nocp len : %02\n", event_datalen);
        for (uint8_t i=0; i < event_datalen; i++)
        {
            printf("%02X ", event_data[i]);
        }
        printf ("\n");*/

/*        start_ptr = &event_data[3U];
        connection_handle = start_ptr[1U];
        connection_handle = connection_handle << 8U;
        connection_handle |= start_ptr[0U];
        connection_handle = connection_handle & 0x0FFFU;

        num_of_completed_pkts = start_ptr[3U];
        num_of_completed_pkts = num_of_completed_pkts << 8U;
        num_of_completed_pkts |= start_ptr[2U];*/
       // UINT8 index = hci_find_cis_entity_by_handle(connection_handle);

      //  printf ("handle %04X,count:%d", connection_handle,num_of_completed_pkts );

        /* Get the current time */
        now_ms = BT_get_time_ms();
       // if ((0U < src_sent_ms) && ((now_ms - src_sent_ms) > 10))
        {
			printf("nocp=%dms, count:%d\n", (now_ms - src_sent_ms), nocp_count);
        }
        src_sent_ms = now_ms;
    }
#endif
...
#define APPL_TRC(...)
        ...
        case HCI_COMMAND_COMPLETE_EVENT:
            ...
            if(HCI_LE_ENHANCED_READ_TRANSMIT_POWER_LEVEL_OPCODE==value_2)
            {
				if (API_SUCCESS == status)
            	{
                    APPL_TRC("\tHCI_LE_ENHANCED_READ_TRANSMIT_POWER_LEVEL SUCCESS\n");
                    hci_unpack_2_byte_param(&connection_handle, event_data);
            		APPL_TRC("\tLE Connection handle          : 0x%04X\n", connection_handle);
                    event_data += 2U;
                    hci_unpack_1_byte_param(&value_1, event_data);
                    APPL_TRC("\tLE_PHY                        : 0x%02X\n", value_1);
                    event_data += 1U;
                    hci_unpack_1_byte_param(&value_1, event_data);
                    APPL_TRC("\tCurrent_Tx_Power_Level  : 0x%02X\n", value_1);
                    event_data += 1U;
                    hci_unpack_1_byte_param(&value_1, event_data);
                    APPL_TRC("\tMax_Tx_Power_Level      : 0x%02X\n", value_1);
            	}
#ifdef APPL_HCI_LE_LEPC_DBGLOG
            	else if (status == 0x11)
            	{
            		APPL_TRC("\t Unsupported Feature or Parameter Value \n");
            	}
            	else if (status == 0x02)
            	{
            		APPL_TRC("\t Unknown Connection Identifier\n");
            		APPL_TRC("\t Connection_Handle parameter does not identify a current ACL connection\n");
            	}
#endif /* APPL_HCI_LE_LEPC_DBGLOG */
            }
            else if(HCI_LE_SET_PATH_LOSS_REPORTING_PARAMETERS_OPCODE ==value_2)
            {
				if (API_SUCCESS == status)
            	{
            		APPL_TRC("\tHCI_LE_SET_PATH_LOSS_REPORTING_PARAMETERS SUCCESS\n");
            		hci_unpack_2_byte_param(&connection_handle, event_data);
            		event_data += 2U;
            		APPL_TRC("\t LE Connection handle         : 0x%04X\n", connection_handle);
            		APPL_TRC("\t Expected Event -HCI_LE_Path_Loss_Threshold event \n");
            	}
#ifdef APPL_HCI_LE_LEPC_DBGLOG
            	else if(status == 0X12)
            	{
            		APPL_TRC("\t Invalid HCI Command Parameters\n");
            	}
            	else if(status == 0X02)
            	{
            		APPL_TRC("\t Unknown Connection Identifier\n");
            ...
            else if(HCI_LE_SET_PATH_LOSS_REPORTING_ENABLE_OPCODE == value_2)
            {
				if (API_SUCCESS == status)
            	{
            		APPL_TRC("\t HCI_LE_Set_Path_Loss_Reporting_Enable SUCCESS\n");
            		hci_unpack_2_byte_param(&connection_handle, event_data);
            		APPL_TRC("\t LE Connection handle         : 0x%04X\n", connection_handle);
            		APPL_TRC("\t If Enable then expected Event -HCI_LE_Path_Loss_Threshold event \n");
            	}
#ifdef APPL_HCI_LE_LEPC_DBGLOG
            	else if(status == 0X0C)
            	{
            		APPL_TRC("\t Set_Path_Loss_Reporting_Enable Issue before Set_Path_loss_Reporting_parameter\n");
            		APPL_TRC("\t Command Disallowed \n");
            	}
            	else if(status == 0X02)
            	{
            		APPL_TRC("\t Unknown Connection Identifier\n");
            ...
            else if(HCI_LE_SET_TRANSMIT_POWER_REPORTING_ENABLE_OPCODE == value_2)
            {
				if (API_SUCCESS == status)
            	{
            		APPL_TRC("\t HCI_LE_Set_Transmit_Power_Reporting_Enable SUCCESS\n");
            		hci_unpack_2_byte_param(&connection_handle, event_data);
            		APPL_TRC("\t LE Connection handle         : 0x%04X\n", connection_handle);
#ifdef APPL_HCI_LE_LEPC_DBGLOG
            		APPL_TRC("\t Expected Event -HCI_LE_Transmit_Power_Reporting event \n");
#endif /* APPL_HCI_LE_LEPC_DBGLOG */
            	}
            }
            /* Check for HCI_LE_SET_ADVERTISING_DATA_OPCODE */
            else if (HCI_LE_SET_ADVERTISING_DATA_OPCODE == value_2)
            {
            }
            ...
            {
                /* 1 byte Num_HCI_Command_Packets
                 * 2 bytes opcode
                 * 1 byte status
                 * 8/9 bytes version*/
                if ((API_SUCCESS == status) && (event_datalen >= 12) && (status == 0))
                {
                  ...
                  if (event_datalen >= 13)
                  {
                      hci_unpack_1_byte_param(&version_info_array[8], event_data + 8U);
                  }
                  appl_print_fw_rom_version(event_datalen);
                }
            }
            else if(HCI_LE_READ_LOCAL_SUPPORTED_FEATURES_OPCODE == value_2)
            {
            	if (API_SUCCESS == status)
            	{
            		//memcpy(local_feature, event_data, 8);
            		if(event_data[3] & 0x70)
            		{
            			local_cis_bis_feature_set=1;
            		}
            	}
            }
#endif /* NXP_CODE */
...
#define APPL_TRC printf
        ...
        case HCI_LE_META_EVENT:
            ...
            case HCI_LE_ADVERTISING_REPORT_SUBEVENT:
                ...
                /**
                 *  For GAP Observer Test Cases, remove below non-connectable check
                ...
                if (BT_FALSE != appl_hci_le_get_auto_conn_state())
                {
                    /* Initiate connection */
...
void appl_hci_le_set_auto_conn_state(UCHAR flag)
{
#ifdef APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV
    appl_hci_le_auto_conn_state = flag;
#else /* APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV */
    BT_IGNORE_UNUSED_PARAM(flag);
#endif /* APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV */
}

UCHAR appl_hci_le_get_auto_conn_state(void)
{
#ifdef APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV
    return appl_hci_le_auto_conn_state;
#else /* APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV */
    return BT_TRUE;
#endif /* APPL_CONFIG_AUTO_CONNECTION_ON_ADV_RECV */
}
