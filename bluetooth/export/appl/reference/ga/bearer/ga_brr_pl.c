...
typedef struct _GA_BRR_DEVICE_BRR_LIST_PL
{
    /*
     * Characteristic context at bearer
     */
    GA_BRR_CHR_CONTEXT char_context;
    ...
    /**
     * Free entity indicator
     */
    UCHAR is_free;

}GA_BRR_DEVICE_BRR_LIST_PL;

typedef struct _GA_BRR_ACTIVITY_CNTX_PL
{
    /**
     * Profile ID context
     */
 ...
 */
DECL_STATIC GA_BRR_DEVICE_BRR_LIST_PL ga_brr_dev_list_pl[GA_BRR_MAX_DEVICE_LIST_COUNT];
...
/* RPA context variables */
DECL_STATIC UCHAR ga_brr_rpbd_addr[BT_BD_ADDR_SIZE];
DECL_STATIC UCHAR ga_brr_rpbd_irk[SMP_IRK_SIZE];
DECL_STATIC UCHAR ga_brr_inrpa = BT_FALSE;

/* Cryptographic context variables */
DECL_STATIC GA_BRR_CRYPTO_CB  ga_brr_crypto_callback;
DECL_STATIC void * ga_brr_crypto_context;
DECL_STATIC UINT8 ga_brr_crypto_op;

#ifdef AES_CMAC_UTIL
DECL_STATIC AES_CMAC_CONTEXT ga_brr_aes_cmac;
DECL_STATIC UCHAR ga_brr_aescmac_mac[AES_CMAC_KEY_SIZE];
DECL_STATIC UCHAR ga_brr_aescmac_key[AES_CMAC_KEY_SIZE];
#endif /* AES_CMAC_UTIL */
...
GA_RESULT ga_brr_remove_from_dev_list_pl
          (
              /* IN  */ ATT_HANDLE * handle
          );
...
void ga_brr_aescmac_complete(void);
          ...
          )
{
    ...
    *id = GA_BRR_MAX_DEVICE_LIST_COUNT;

    for (i = 0; i < GA_BRR_MAX_DEVICE_LIST_COUNT; i++)
    {
        if ((BT_TRUE != ga_brr_dev_list_pl[i].is_free) &&
            (ga_brr_dev_list_pl[i].att_handle.att_id == handle->att_id) &&
    ...
    *id = GA_BRR_MAX_DEVICE_LIST_COUNT;
    ...
    if (GA_BRR_DEV_ADDR_IS_NON_ZERO(ga_dev->bd_addr))
    {
        for (i = 0; i < GA_BRR_MAX_DEVICE_LIST_COUNT; i++)
        {
            if (BT_TRUE != ga_brr_dev_list_pl[i].is_free)
            {
                retval = ga_get_bd_addr_from_dev_hndl(&ga_brr_dev_list_pl[i].att_handle.device_id, &bd_addr);
                ga_get_ga_dev_from_bd_addr(&bd_addr, NULL, &brr_dev);

                if ((GA_SUCCESS == retval) &&
                    (GA_COMPARE_BRR_DEVICE(&brr_dev, ga_dev)))
                {
                    *id = (UCHAR)i;
                    retval = GA_SUCCESS;
    ...
    for (i = 0; i < GA_BRR_MAX_DEVICE_LIST_COUNT; i++)
    {
        if (BT_TRUE == ga_brr_dev_list_pl[i].is_free)
        {
            *id = i;
            ...
            ga_brr_dev_list_pl[i].is_free = BT_FALSE;
          ...
          )
{
    ...
    if (GA_SUCCESS == ga_brr_search_dev_list_by_handle_pl(handle, &id))
    {
        ...
        ga_brr_dev_list_pl[id].is_free = BT_TRUE;
    }
...
GA_RESULT ga_brr_init_pl(void)
{
    ...
    for (i = 0; i < GA_BRR_MAX_DEVICE_LIST_COUNT; i++)
    {
        ...
        ga_brr_dev_list_pl[i].is_free = BT_TRUE;
    }
    ...
    /* Initialize the crypto op */
    ga_brr_crypto_op = GA_BRR_CRYPTO_NONE;
...
void ga_brr_set_default_broadcast_settings (void)
{
    ...
    BT_mem_set(&ga_brr_eadv_params.own_addr[0], 0, BT_BD_ADDR_SIZE);
}
          ...
          )
{
    ...
    exevent = GA_BRR_CONFIGURE_ADV_CNF;
    ...
    else
    {
        if (GA_BRR_ADV_TYPE_PA == type)
        {
            ...
            else
            {
                ...
                if (API_SUCCESS == retval)
                {
                    exevent = GA_BRR_RECONFIGURE_ADV_CNF;
                }
            }
        }
        else if (GA_BRR_ADV_TYPE_EA == type)
        {
            if (BT_TRUE == enable)
            {
                ...
                if ((GA_SUCCESS == retval) && (NULL != data))
                {
                    ...
                    {
                        /*
                         * If the own_addr_type is 1(Random), then set the
                         * random address for the handle.
                         * The sequence has to be followed as below.
                         */
                        if (1U == ga_brr_eadv_params.own_addr_type)
                        {
                            /* Handle the event */
                            retval = BT_hci_le_set_adv_set_random_address
                                     (
                                         handle,
                                         ga_brr_eadv_params.own_addr
                                     );
                        }
    ...
    if (GA_SUCCESS == retval)
    {
        ...
        ga_brr_act_ctx[id].expt_event = exevent;
          ...
          )
{
    ...
    /* TODO: Currently used by only Profile, but it could be used by both Profile and Server */
          ...
          )
{
    ...
    if (NULL == addr)
    {
        BT_mem_copy(ga_brr_rpbd_irk, irk, SMP_IRK_SIZE);
        retval = BT_smp_create_resolvable_pvt_addr(ga_brr_rpbd_irk);
    }
    else
    {
        BT_COPY_BD_ADDR(ga_brr_rpbd_addr, addr);
        BT_mem_copy(ga_brr_rpbd_irk, irk, SMP_IRK_SIZE);
        retval = BT_smp_verify_resolvable_pvt_addr(ga_brr_rpbd_addr, ga_brr_rpbd_irk);
    }

    if (API_SUCCESS == retval)
    {
        ga_brr_inrpa = BT_TRUE;
    }
...
{
    GA_BRR_AESCMAC_INFO * aescmac;
    API_RESULT retval;

    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: >> ga_brr_crypto\n");
    ...
    if (GA_BRR_CRYPTO_NONE != ga_brr_crypto_op)
    {
        /* Unlock */
        BRR_UNLOCK();
...
#ifdef AES_CMAC_UTIL
    /* Curently supporting only for AES-CMAC */
    if (GA_BRR_CRYPTO_AESCMAC == op)
    {
        /* Initialize module */
        (GA_IGNORE_RETURN_VALUE)aes_cmac_init();

        /* Initialize context for CMAC signing */
        (GA_IGNORE_RETURN_VALUE)aes_cmac_context_init(&ga_brr_aes_cmac);

        aescmac = (GA_BRR_AESCMAC_INFO *)info;

        /* Allocate and copy the plain text */
        if (0U != aescmac->textlen)
        {
            ga_brr_aes_cmac.data = BT_alloc_mem(aescmac->textlen);
            if (NULL != ga_brr_aes_cmac.data)
            {
                BT_mem_copy(ga_brr_aes_cmac.data, aescmac->text, aescmac->textlen);
            }
        }

        /* Reverse the Input and Reverse the Key? */

        /* Update context parameters */
        ga_brr_aes_cmac.action = (0x01U != aescmac->verify) ?
            AES_CMAC_MAC_GENERATE : AES_CMAC_MAC_VERIFY;

        ga_brr_aes_cmac.maclen = aescmac->maclen;
        ga_brr_aes_cmac.datalen = aescmac->textlen;

        ga_brr_aes_cmac.key = ga_brr_aescmac_key;
        BT_mem_copy(ga_brr_aes_cmac.key, aescmac->key, AES_CMAC_KEY_SIZE);
        ga_brr_aes_cmac.maclen = aescmac->maclen;

        ga_brr_aes_cmac.mac = ga_brr_aescmac_mac;
        ga_brr_aes_cmac.cb = ga_brr_aescmac_complete;

        /* Start generation/verification */
        retval = aes_cmac(&ga_brr_aes_cmac);

        if (AES_CMAC_PROC_PENDING == retval)
        {
            ga_brr_crypto_op = op;
            ga_brr_crypto_callback = cb;
            ga_brr_crypto_context = context;
...
void ga_brr_aescmac_complete(void)
{
    UCHAR mac[AES_CMAC_KEY_SIZE];
    UINT16 maclen, status;
    UINT8 evt;
    GA_BRR_CRYPTO_CB cb;
    void * context;
    ...
    if (NULL != ga_brr_aes_cmac.data)
    {
        BT_free_mem(ga_brr_aes_cmac.data);
    }

    /* Reset the Op. Not checking if valid */
    ga_brr_crypto_op = GA_BRR_CRYPTO_NONE;

    /* Get local copy of the context data */
    cb = ga_brr_crypto_callback;
    context = ga_brr_crypto_context;

    /* Get a local copy of the Output data */
    BT_mem_copy(mac, ga_brr_aescmac_mac, sizeof(mac));
    maclen = ga_brr_aes_cmac.maclen;

    /* Set the event status */
    status = (0x00U == ga_brr_aes_cmac.state) ? GA_SUCCESS : GA_FAILURE;
    evt = (AES_CMAC_MAC_GENERATE == ga_brr_aes_cmac.action)?
        GA_BRR_CRYPTO_AESCMAC_CREATE_COMPLETE: GA_BRR_CRYPTO_AESCMAC_VERIFY_COMPLETE;
    ...
    {
        /* Notify Upper Layer */
        cb(evt, status, mac, maclen, context);
    }
}
          ...
          )
{
    ...
    retval     = GA_FAILURE;
    id         = GA_BRR_MAX_ACTIVITY_CNTX;
    pl_id      = GA_BRR_MAX_DEVICE_LIST_COUNT;
    create_ctx = BT_TRUE;
    ...
    */
    if (API_SUCCESS == retval)
    {
        ...
        case GA_SERVICE_ASCS:
            sdp_inst = DB_RECORD_ASCS;
            break;
        case GA_SERVICE_BASS:
            sdp_inst = DB_RECORD_BASS;
            break;
        case GA_SERVICE_CAS:
            sdp_inst = DB_RECORD_CAS;
            break;
        ...
        if (0xFF != sdp_inst)
        {
            ...
            retval = BT_dbase_get_record_handle(sdp_inst, 0U, &record_handle);
...
GA_RESULT ga_brr_get_service_security_requirement
          (
              /* IN */  GA_BRR_UUID s_uuid,
              /* OUT */ GATT_DB_SERVICE_DESC * desc
          )
{
    GA_RESULT retval;
    ...
    /* Include any Service UUID checks that do not require security */
    if (GA_SERVICE_TMAS == s_uuid)
    {
        *desc = GATT_DB_SER_NO_SECURITY_PROPERTY;
        retval = GA_SUCCESS;
    }
          ...
          )
{
    ...
    /* Override default security requirement if specified at bearer */
    retval = ga_brr_get_service_security_requirement(service_uuid, &desc);
    if (API_SUCCESS == retval)
    {
        service_info.sec_req = desc;
    }
                       ...
                       )
{
    ...
    /* Default value of CCCD or SCCD */
    UCHAR ccd_value[2U]    = { 0x00U, 0x00U };

    /* Initialize Permission */
    perm                   = (GATT_DB_PERM_READ | GATT_DB_PERM_WRITE);

    /* Assign UUID as CCCD or SCCD based on the "is_cccd" flag */
          ...
          )
{
    ...
    /* If resposne required is false, would be because of ReadByType for UUID */
    if (GA_TRUE != ctx->to_rsp)
    {
        ctx->status = status;

        if (GA_SUCCESS == status)
        {
            ctx->length = value->len;
            BT_mem_copy(ctx->value, value->val, value->len);
        }
        else
        {
            ctx->length = 0U;
...
/* ----------------------------------------------------------------------- */
void ga_rpa_complete(UCHAR evt, API_RESULT status, UCHAR * data, UINT16 datalen)
{
    GA_BRR_PRF_HANDLE pid;
    UCHAR ga_evt;

    if (BT_TRUE != ga_brr_inrpa)
    {
        return;
    }

    ga_brr_inrpa = BT_FALSE;

    if (NULL != ga_brr_pl_cb)
    {
        ga_evt = (SMP_RESOLVABLE_PVT_ADDR_CREATE_CNF == evt) ?
            GA_BRR_RPA_CREATE_COMPLETE : GA_BRR_RPA_VERIFY_COMPLETE;

        pid = GA_BRR_INST_DONTCARE;
...
GA_RESULT ga_hci_event_callback(UINT8 event_code, UINT8* event_data, UINT8 event_datalen)
{
    ...
    case HCI_COMMAND_COMPLETE_EVENT:
        ...
        if (event_datalen >= 4)
        {
            ...
            case HCI_LE_SET_ADVERTISING_SET_RANDOM_ADDRESS_OPCODE:
                break;
            ...
            case HCI_LE_SET_PERIODIC_ADVERTISING_DATA_OPCODE:
                if ((GA_BRR_CONFIGURE_ADV_CNF == ga_brr_act_ctx[id].expt_event) ||
                    (GA_BRR_RECONFIGURE_ADV_CNF == ga_brr_act_ctx[id].expt_event))
                {
                    /* Trigger to the above layer with failure status */
                    if (0x0 != status)
                    {
                    ...
                    else
                    {
                        ...
                        /* If PA is already enabled, and data is updated generate event here */
                        if (GA_BRR_RECONFIGURE_ADV_CNF == ga_brr_act_ctx[id].expt_event)
                        {
                            del_act = GA_TRUE;
                            ga_event = ga_brr_act_ctx[id].expt_event;
                        }
                    ...
                    (GA_BRR_RECONFIGURE_ADV_CNF == ga_brr_act_ctx[id].expt_event))
                {
                    ...
                    else
                    {
                         ...
                         */
                         ...
                         /* If EA is already enabled, and data is updated generate event here */
                        if (GA_BRR_RECONFIGURE_ADV_CNF == ga_brr_act_ctx[id].expt_event)
                        {
                            del_act = GA_TRUE;
                            ga_event = ga_brr_act_ctx[id].expt_event;
            ...
            case HCI_LE_ENCRYPT_OPCODE:
#ifdef AES_CMAC_UTIL
                /* Check for any bearer initiated Crypto operation to be processed */
                if (GA_BRR_CRYPTO_AESCMAC == ga_brr_crypto_op)
                {
                    /* Unlock */
                    BRR_UNLOCK();

                    /* Invoke AESCMAC modules encryption complete */
                    aes_cmac_128B_encrypt_complete_pl(value_1, status, event_data, (event_datalen - 4U));
    ...
    case HCI_LE_META_EVENT:
        ...
        case HCI_LE_EXTENDED_ADVERTISING_REPORT_SUBEVENT:
            ...
            for (index = 0; index < value_1; index++)
            {
                ...
                {
                    CONSOLE_OUT("*** Adv report incorrectly parsed ***\n");
                    break;
                }
            ...
            ga_event = GA_BRR_EVENT_INVALID;
            break;
        case HCI_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED_SUBEVENT: /* Fall Through */
#ifdef BT_5_4
        case HCI_LE_PERIODIC_ADVERTISING_SYNC_ESTABLISHED_V2_SUBEVENT:
#endif /* BT_5_4 */
            if (GA_BRR_CONFIGURE_SYNC_CNF == ga_brr_act_ctx[id].expt_event)
            {
                hci_unpack_1_byte_param(&status, event_data);
          ...
          )
{
    ...
    del_act          = BT_FALSE;
    char_hndl        = 0;
    GA_mem_set(&ga_dev, 0U, sizeof(GA_BRR_DEVICE));
    ...
    if (ATT_CONNECTION_IND != att_event)
    {
         ...
         */
        if ((ATT_HANDLE_VALUE_NTF != att_event) &&
            (ATT_HANDLE_VALUE_IND != att_event) &&
            (ATT_DISCONNECTION_IND != att_event))
        {
            ...
            if (GA_SUCCESS != retval)
            {
                ...
                GA_BRR_DEVICE_PRINT_STR(&ga_dev));
    ...
    case ATT_DISCONNECTION_IND:
        ...
        /* Delete any active contexts for the device */
        retval = ga_brr_find_activity_by_dev_pl
                 (
                     &ga_dev,
                     &id
        ...
        /* Set to non-bonded device information by default */
        dev.bonded = GA_FALSE;

        /* Check for device bonding status */
        BT_smp_get_bd_handle(&bd_addr, &bd_handle);
        retval = BT_smp_get_device_security_info(&bd_handle, &auth);
        if ((API_SUCCESS == retval) &&
            (SMP_BONDING == auth.bonding))
        {
            dev.bonded = GA_TRUE;
        }

        ga_event_data    = &dev;
        ga_event_datalen = sizeof(GA_BRR_DEVICE_INFO);
    ...
    /* Is transport disconnection? */
    if (ATT_DISCONNECTION_IND == att_event)
    {
        /* Remove from the device list */
        ga_brr_remove_from_dev_list_pl(handle);
    }
           ...
           )
{
    ...
    char_cntx = &ga_brr_dev_list_pl[pl_id].char_context;
    ...
    switch (params->db_op)
    {
        ...
        case GATT_DB_CHAR_PEER_READ_BY_TYPE_REQ:
            GA_BRR_PL_TRC(
            ...
            char_cntx->uuid   = uuid;
            char_cntx->inst   = handle->char_id;
            char_cntx->to_rsp = (GATT_DB_CHAR_PEER_READ_REQ == params->db_op)? GA_TRUE: GA_FALSE;
            break;
        ...
        case GATT_DB_CHAR_PEER_EXECUTE_WRITE_REQ:
            GA_BRR_PL_TRC(
                "[GA_BRR_PL]: Received Exectute Write for Handle: for Handle: 0x%04X and "
                " UUID: 0x%04X\n", params->handle, uuid);
            ...
            /* Assign the UL Context for this event */
            BT_UNPACK_LE_2_BYTE(&char_cntx->uuid, GATT_DB_GET_UUID(params->handle));
            char_cntx->inst = handle->char_id;
            char_cntx->to_rsp = GA_TRUE;
            char_cntx->is_blob = GA_TRUE;
    ...
    if ((NULL != ga_brr_pl_cb) && (BT_TRUE == invoke_cb))
    {
        ...
        char_cntx->inst, GA_BRR_DEVICE_PRINT_STR(&ga_dev),
        ...
        else if (GATT_DB_CHAR_PEER_READ_BY_TYPE_REQ == params->db_op)
        {
            retval = char_cntx->status;
            params->value.actual_len = char_cntx->length;
            params->value.len = char_cntx->length;
            params->value.val = char_cntx->value;

            retval = GATT_DB_UL_UPDATED_ATTR_VAL;
        }
    }
...
GA_RESULT ga_brr_setup_pl(void)
{
    ...
    {
#ifdef BT_GAM_ENABLE_ON_BREDR
        /* Update the Device Class with GA Major Class */
        appl_hci_write_classofdevice(BT_MSC_LE_AUDIO);
...
GA_RESULT ga_brr_config_escan_params_pl
          (
          ...
          )
{
    ...
    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: >> ga_brr_config_escan_params_pl\n");

    if (NULL != escan_params)
    {
        ga_brr_escan_params = *escan_params;
        retval = GA_SUCCESS;
    }

    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: << ga_brr_config_escan_params_pl, retval 0x%04X\n", retval);

    return retval;
}

GA_RESULT ga_brr_config_eadv_params_pl
          (
          ...
          )
{
    ...
    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: >> ga_brr_config_eadv_params_pl\n");

    if (NULL != eadv_params)
    {
        ga_brr_eadv_params = *eadv_params;
        retval = GA_SUCCESS;
    }

    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: << ga_brr_config_eadv_params_pl, retval 0x%04X\n", retval);

    return retval;
}

GA_RESULT ga_brr_config_psync_params_pl
          (
          ...
          )
{
    ...
    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: >> ga_brr_config_psync_params_pl\n");

    if (NULL != psync_params)
    {
        ga_brr_psync_params = *psync_params;
        retval = GA_SUCCESS;
    }

    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: << ga_brr_config_psync_params_pl, retval 0x%04X\n", retval);

    return retval;
}

GA_RESULT ga_brr_config_padv_params_pl
          (
          ...
          )
{
    ...
    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: >> ga_brr_config_padv_params_pl\n");

    if (NULL != padv_params)
    {
        ga_brr_padv_params = *padv_params;
        retval = GA_SUCCESS;
    }

    GA_BRR_PL_TRC(
    "[GA_BRR_PL]: << ga_brr_config_padv_params_pl, retval 0x%04X\n", retval);
