          ...
          )
{
    ...
    if (AUDIO_LC3_IDLE == audio_lc3_snk.state)
    {
        ...
        {
            for (uint8_t chan_index = 0; (!lc3_retval) && (chan_index < cc); chan_index++)
            {
            	lc3_retval = lc3_dec_init_ch_cntx
            				(
            					&audio_lc3_snk.ctx.dec_ctx[chan_index],
								&audio_lc3_snk.ctx.config,
								audio_lc3_snk.ctx.config.n_f_len  //TODO: Verify this param
							);
            }
        }
        ...
        else
        {
            ...
            (
                ...
                0U,
                sizeof(audio_lc3_snk.ctx.dec_ctx)
            );
            audio_lc3_snk.state = AUDIO_LC3_IDLE;
        }
    }
...
GA_RESULT audio_snk_lc3_decode_n_play(UCHAR * data, UINT16 datalen)
{
    ...
    samples_per_frame_per_channel = audio_lc3_snk.ctx.config.n_f_len;


    for (uint8_t chan_ind = 0; chan_ind < audio_lc3_snk.cc; chan_ind++)
    {
        retval = LC3_decode_a_frame
					(
						&audio_lc3_snk.ctx.dec_ctx[chan_ind],
						&audio_lc3_snk.ctx.snk_enc_buffer[chan_ind][0],
						&audio_lc3_snk.ctx.snk_dec_buffer[chan_ind][0],
						flg_bytes[chan_ind],
