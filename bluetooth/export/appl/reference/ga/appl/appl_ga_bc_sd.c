...
================ Service Management ~ PAST and Streaming Procedures ============ \n\
   ...
   18. Enable Stream. \n\
   19. Disable Stream. \n\
   20. Associate to Broadcast Source (Autonomous Procedure). \n\
\n\
...
typedef struct _appl_ga_bc_sd_bass_local_db
{
    ...
    /* Remote Scan Status */
    UINT8 scan_status;
    /* Is Auto - Used for checking if the procedure is done autonomously by SD */
    UINT8 is_auto;
    /* BC Source Endpoint Info - PAST Not Available and Autonomous */
...
APPL_BC_SD_BC_SRC_INFO bc_src_db[APPL_GA_BC_SD_SNK_REM_SRC_DB_COUNT];
...
UCHAR appl_ga_bc_snk_bc_code[GA_BROADCAST_CODE_SIZE];
...
void appl_ga_bc_sd_snk_add_rem_src_to_db(UINT8 *addr, UINT8 addr_type, UINT8 sid, UINT32 bc_id);
void appl_ga_bc_sd_snk_clear_rem_src_db(void);
UINT8 appl_ga_bc_sd_snk_is_rem_src_in_db(UINT8* addr, UINT8 addr_type, UINT8 * db_index);
...
#endif /* BASS_DB */
void appl_ga_bc_sd_get_sg_bis_info_index_and_setup_iso(void);
void appl_ga_bc_sd_get_sg_bis_info_index_and_remove_iso(void);

void appl_ga_bc_sd_snk_display_bis_stream_info
     (
         UINT8  sg_info_index,
         UINT8  bis_info_index
     );
            ...
            if (event_datalen >= 4U)
            {
                ...
                case HCI_LE_SETUP_ISO_DATA_PATH_OPCODE:
                            ...
                            ))
                    {
                        ...
                        if (snk_param->bis_index_curr_iso_enable_disable < snk_param->bis_count_enable)
                        {
                            /*
                             * Trigger setup ISO data path for next valid
                             * BIS index since we havent completed setup ISO
                             * for all Synced BISes.
                             */
                            appl_ga_bc_sd_get_sg_bis_info_index_and_setup_iso();
                        }
                        else
                        {
                            /*
                             * There are no more BIS for ISO Data Path Setup
                             * Remove reference of old BIS index enable[]
                             */
                            snk_param->bis_index_curr_iso_enable_disable = 0U;
                        }
                    }
                    break;
                ...
                case HCI_LE_REMOVE_ISO_DATA_PATH_OPCODE:
                            ...
                            ))
                    {
                        ...
                        {
                            /*
                             * There are more BIS for Removal of ISO Data
                             * Path
                             */
                            appl_ga_bc_sd_get_sg_bis_info_index_and_remove_iso();
                        }
                        else
                        {
                            /*
                             * There are no more BIS for ISO Data Path Remove
                             * Remove reference of old BIS index enable[]
                             */
                            snk_param->bis_index_curr_iso_enable_disable = 0U;
                        }
                ...
                case HCI_LE_SET_PERIODIC_ADVERTISING_SYNC_TRANSFER_PARAMETERS_OPCODE:
                    ...
                    if (0x00U == status)
                    {
                                          ...
                                          ))
                        {
                             ...
                             * NOTE:
                             * Timeout = 9sec
                             * (4sec Add_Source/Modify_Source timeout + 5sec PAST receiving timeout)
                             */
                            BT_start_timer
                            (
        ...
        switch (sub_event)
        {
            case HCI_LE_PERIODIC_ADVERTISING_SYNC_TRANSFER_RECEIVED_SUBEVENT:
            {
                ...
                if (0x00U == le_periodic_advertising_sync_transfer_received_subevent.status)
                {
                    ...
                    APPL_TRC
                    (
                        "Advertiser Address Type: 0x%02X\n",
                        le_periodic_advertising_sync_transfer_received_subevent.advertiser_address_type
                    );
                    marker++;
                    device.bd_type =
                    ...
                    le_periodic_advertising_sync_transfer_received_subevent.advertiser_address =
                        &event_data[marker];
                    APPL_TRC("Advertiser Address: ");
                    for (i = 0U; i < 6U; i++)
                    {
                        APPL_TRC("%02X ", le_periodic_advertising_sync_transfer_received_subevent.advertiser_address[i]);
                    }
                    APPL_TRC("\n");
                    marker += 6U;
                    ...
                    APPL_TRC
                    (
                        "Periodic Advertising Interval: 0x%04X\n",
                        le_periodic_advertising_sync_transfer_received_subevent.periodic_advertising_interval
                    );
                    marker += 2U;
                    ...
                    APPL_TRC
                    (
                        "Adveriser Clock Accuracy: 0x%02X\n",
                        le_periodic_advertising_sync_transfer_received_subevent.advertiser_clock_accuracy
                    );
                    marker++;
          ...
          )
{
    ...
    case GA_BC_SOURCE_ANNOUNCEMENT_IND:
    {
            ...
            ))
        {
            ...
            {
                if (GA_TRUE == bc_annoucement.pbp_found)
                {
#ifdef APPL_GA_PBP
                    PBP_DECODE_BCAST_ANNOUNCEMENT_FEATURES
                    (
                        bc_annoucement.pbp_features,
                        en,
                        sq,
                        hq
                    );

                    /* Display the Encryption Status, High Quality, Standard Quality */

                    if ((0U != bc_annoucement.pbp_metadata_len) && (NULL != bc_annoucement.pbp_metadata))
                    {
    ...
    case GA_BC_SOURCE_CONFIG_IND:
                          ...
                          ))
        {
            ...
            /* Check if SD has triggered autonomous procedure */
            if (GA_TRUE == appl_ga_bc_sd_bass_local_db.is_auto)
            {
                /* Check if the Source can be added */
                if (GA_SUCCESS == appl_ga_bc_sd_is_src_add_allowed(&index))
                {
                    /* Use this unique index as Source_ID */
                    /* Check if the Source is present in the DB */
                    if (GA_TRUE == appl_ga_bc_sd_snk_is_rem_src_in_db
                                   (
                                       appl_ga_bc_sd_bass_local_db.bc_src_dev.bd_addr,
                                       appl_ga_bc_sd_bass_local_db.bc_src_dev.bd_type,
                                       &db_index
                                   ))
                    {
                        ...
                        /* Source_Address_Type */
                        appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.addr_type =
                            bc_src_db[db_index].adv_addr_type;
                        marker++;
                        ...
                        (
                            appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.addr,
                            bc_src_db[db_index].adv_addr
                        );
                        marker += GA_BD_ADDR_SIZE;

                        /* Source_Adv_SID */
                        appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.adv_sid =
                            bc_src_db[db_index].adv_SID;
                        marker++;

                        /* Broadcast_ID */
                        GA_mem_copy
                        (
                            appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.broadcast_ID,
                            bc_src_db[db_index].broadcast_ID,
                            BASS_BC_ID_LEN
                        );
                        ...
                        appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.num_subgroups =
                            appl_ga_bc_sd_params.bc_num_subgroups;
                        marker++;

                        /* Give reference of arrays to the pointers */
                        ...
                        /* If the num_subgroups is greater than 1, then process subgroup_info */
                        if (1U <= appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.num_subgroups)
                        {
                            for (i = 0U; i < appl_ga_bc_sd_params.bc_num_subgroups; i++)
                            {
                                /*
                                 * BIS_Sync State[i]
                                 * Update BIS_Sync_State to 0 here. Will be
                                 * updated in GA_BC_ENABLE_CNF event
                                 */
                                appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.bis_sync_state[i] = 0U;
                                marker += 4U;

                                /* Metadata_Length[i] */
                                appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.metadata_len[i] =
                                    appl_ga_bc_sd_params.bc_subgroup_info[i].bc_basic_announcement_1.meta.length;
                                marker++;

                                /* Metadata[i] */
                                GA_mem_copy
                                (
                                    &appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.metadata[i],
                                    &appl_ga_bc_sd_params.bc_subgroup_info[i].bc_basic_announcement_1.meta.data[0U],
                                    appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.metadata_len[i]
                                );
                                marker += appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.metadata_len[i];
                            }
                        }
                        /* Update length of BC_RX_State */
                        appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info_len = marker;
                        /* Add the RX State to DB */
#ifdef BASS_DB
                        ...
                        if (GA_SUCCESS == retval)
                        {
                            APPL_TRC
                            (
                                "Source_ID of the added source: 0x%04X\n",
                                index
                            );
                            ...
                            /* Extract num_subgroups and bis_sync of each subgroups. Used during enable. */
                            appl_ga_bc_sd_bass_local_db.rx_state_inst[index].subgroup = appl_ga_bc_sd_params.bc_num_subgroups;
                            for (sg_info_index = 0U ; sg_info_index < appl_ga_bc_sd_params.bc_num_subgroups; sg_info_index++)
                            {
                                ...
                                {
                                    if (0U != appl_ga_bc_sd_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_index)
                                    {
                                        BASS_BIS_SYNC_STATE_UPDATE_SYNC_TO_BIS_INDEX
                                        (
                                            appl_ga_bc_sd_bass_local_db.rx_state_inst[index].bis_sync[sg_info_index],
                                            appl_ga_bc_sd_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_index
                                        );
                                    }
                                }
                ...
                appl_ga_bc_sd_bass_local_db.is_auto = GA_FALSE;
            }
            else
            {
                /*
            ...
            GA_broadcast_decode_event_params(GA_BC_ENABLE_CNF, ga_data, &bc_enable))
        {
            ...
            if (0x00U == bc_enable.error_code)
            {
                ...
                if (0U < bc_enable.num_bis)
                {
                    ...
                    /*
                     * If appl_ga_bc_sink_params.bis_count_enable > 1, then
                     * this is a case where we are trying to Sync to more than 1
                     * BIS at a time.
                     */

                    /*
                     * Perform Setup ISO Data path in a chain and also start
                     * receiving Audio for a single BIS
                     ...
                     * a file.
                     */
                    if (appl_ga_bc_sd_params.bis_index_curr_iso_enable_disable < appl_ga_bc_sd_params.bis_count_enable)
                    {
                        /*
                         * Fetch the first valid SG Index and BIS Index to
                         * Setup ISO Data by referring to first BIS index in
                         * the bis_index_enable[] and Setup ISO Data Path
                         * for the same.
                         */
                        appl_ga_bc_sd_get_sg_bis_info_index_and_setup_iso();
                    }
    ...
    case GA_BC_DISSOCIATE_IND:
                          ...
                          ))
        {
                              ...
                              ))
            {
                ...
                {
                    /* In case where GA_BC_DISSOCIATE_IND is received first and then
                     * GA_BC_DISABLE_IND is received/not received, Ensure to update the
                     * BIS Sync State since PA Sync is already Lost.
                     */
                    appl_ga_bc_sd_bass_bc_rx_state.bc_rx_state.info.bis_sync_state[i] =
                        BASS_BIS_SYNC_STATE_NOT_SYNCED_TO_BIS;
                }
    ...
    case GA_BC_DISABLE_CNF:
        ...
        if (GA_SUCCESS == ga_status)
        {
            ...
            /*
             * If > 1 BIS is enabled and ISO is setup for all,
             * Call Remove ISO Data path in a chain and also stop
             * receiving Audio for bis_index_active_playback
             * i.e. Stop LC3 Decoder Module + Audio PL Module only
             * once while ISO Data Path can be remove for all.
             */
            for (i = 0U; i < appl_ga_bc_sd_params.bis_count_enable; i++)
            {
                /*
                 * Fetch the valid SG Index and BIS Index to Setup ISO Data
                 * by referring to BIS index in the bis_index_enable[]
                 */
                if (GA_SUCCESS ==
                    appl_ga_bc_sd_snk_fetch_sg_bis_info_index_from_bis_index
                    (
                ...
                {
                    /*
                     * As BIG is already terminated when this event is
                     * generated, update the entry for ISO Removal if any
                     * Remove ISO Entry Failed.
                     */
                    if (AUDIO_ISO_IDLE !=
    ...
    case GA_BC_DISABLE_IND:
        ...
        if (GA_SUCCESS == ga_status)
        {
            ...
            {
                /*
                 * Fetch the valid SG Index and BIS Index to Remove ISO Data
                 * by referring to BIS index in the bis_index_enable[]
                 */
                if (GA_SUCCESS ==
                    appl_ga_bc_sd_snk_fetch_sg_bis_info_index_from_bis_index
                    ...
                    ))
                {
                    /*
                     * As BIG is already terminated when this event is
                     * generated, update the entry for ISO Removal.
                     */
                    if (GA_SUCCESS == appl_ga_utils_audio_snk_remove_entry
                                      (
                                          appl_ga_bc_sd_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_handle
            ...
            ))
        {
                              ...
                              ))
            {
                                ...
                                ))
                {
                    ...
                    (
                        "Len: 0x%02X,  Broadcast_ID: 0x%06X\n",
                        GA_BC_ID_LEN, bc_annoucement.broadcast_id
                    );
                    appl_ga_utils_parse_and_display_adv_data
                    (
                        bc_source_announcement.data,
            ...
            GA_broadcast_decode_event_params(GA_BC_ENABLE_CNF, ga_data, &bc_enable))
        {
            ...
            if (0x00U == bc_enable.error_code)
            {
                ...
                if (0U < bc_enable.num_bis)
                {
                     ...
                     * a file.
                     */
                    if (appl_ga_bc_sink_params.bis_index_curr_iso_enable_disable < appl_ga_bc_sink_params.bis_count_enable)
                    {
                        /*
                         * Fetch the first valid SG Index and BIS Index to
                         * Setup ISO Data by referring to first BIS index in
...
void appl_ga_bc_snk_init(void)
{
    ...
    appl_ga_bc_sink_params.bis_index_curr_iso_enable_disable = 0U;
...
void appl_ga_bc_sd_init(void)
{
    ...
    appl_ga_bc_sd_params.bis_index_curr_iso_enable_disable = 0U;
...
void main_sd_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        switch(menu_choice)
        {
            ...
            case 3:
                if (BC_ROLE_SD != (BC_ROLE_SD & bc_role))
                {
                    APPL_TRC("SD role is not registered for the device. "
            ...
            case 7:
                appl_ga_utils_config_brr_escan();
                break;
            ...
            case 18:
                    ...
                    ))
                {
                                      ...
                                      ))
                    {
                        ...
                        APPL_TRC("SD - GA_broadcast_enable...\n");
            ...
            case 19:
                 ...
                 */
                /*
                 * Fetch the first valid SG Index and BIS Index to Remove ISO
                 * Data by referring to first BIS index in the bis_index_enable[]
                 * and Remove ISO Data Path for the same.
                 */
                appl_ga_bc_sd_get_sg_bis_info_index_and_remove_iso();
            ...
            case 20:
                ...
                if (GA_SUCCESS == retval)
                {
                    appl_ga_bc_sd_bass_local_db.bc_src_dev.bd_type = device.bd_type;
                    GA_COPY_BRR_DEV_ADDR
                    (
                        appl_ga_bc_sd_bass_local_db.bc_src_dev.bd_addr,
                        device.bd_addr
                    );
                    appl_ga_bc_sd_bass_local_db.is_auto = GA_TRUE;
                }
                break;
            ...
            case 22:
                ...
                (
                    &bc_src_db[0U],
                    0U,
                    (sizeof(APPL_BC_SD_BC_SRC_INFO) * APPL_GA_BC_SD_SNK_REM_SRC_DB_COUNT)
                );
                rem_src_db_count = 0U;
                GA_mem_set
                (
                    &appl_ga_bc_sink_params,
                    0U,
                    sizeof(BC_SINK_PARAMS)
                );
                appl_ga_bc_sd_init();

                GA_mem_set
                (
                    &appl_ga_bc_sd_bass_local_db.rx_state_inst[0U],
                    0U,
                    (sizeof(APPL_GA_BC_SD_RX_STATE_INST) * BASS_SE_MAX_BC_RX_STATE_ENTITIES)
                );

#ifdef BASS_DB
                APPL_TRC("BASS_DB_remove_source()...\n");
                retval = BASS_DB_remove_source(0U);
                APPL_TRC("Retval - ");
                appl_ga_bc_sd_bass_display_ga_result(retval);
#endif /* BASS_DB */
                break;
            ...
            case 31:
                ...
                {
                    CONSOLE_OUT("Enter Broadcast_Code [IN HEX]: ");
                    for (i = 0U; i < GA_BROADCAST_CODE_SIZE; i++)
                    {
                        CONSOLE_IN("%x", &choice);
          ...
          )
{
    ...
    else
    {
        ...
        if (GA_SUCCESS == retval)
        {
            ...
            /*
             * Copy the BC_Src address and type to the local DB. Required to
             * handle Sync_Info request and No_PAST cases, and also
             * modify_source.
             */
            GA_COPY_BRR_DEV_ADDR
            (
            ...
            /*
             * Send notification only when PA_Sync is not
          ...
          )
{
    ...
    else
    {
        ...
        if (((GA_TRUE == is_enable_required) &&
            (BASS_PA_SYNC_SYNC_TO_PA_PAST_AVL != modify_src.pa_sync)) ||
            (GA_TRUE == is_disable_required))
        {
        ...
        (AUDIO_ISO_IDLE == snk_param->bc_subgroup_info[sg_info_index].bis_info[bis_info_index].iso_state))
    {
                          ...
                          ))
        {
            retval = appl_ga_utils_audio_snk_le_setup_iso_data_path
                     (
                         snk_param->bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_handle,
                         &snk_param->bc_subgroup_info[sg_info_index].bis_info[bis_info_index].stream_symbol
                     );
...
void appl_ga_bc_sd_snk_add_rem_src_to_db(UINT8 *addr, UINT8 addr_type, UINT8 sid, UINT32 bc_id)
{
    ...
    if (rem_src_db_count < APPL_GA_BC_SD_SNK_REM_SRC_DB_COUNT)
    {
        ...
        {
            bc_src_db[rem_src_db_count].adv_addr[index] = *(addr + index);
        }
        bc_src_db[rem_src_db_count].adv_addr_type = addr_type;
        bc_src_db[rem_src_db_count].adv_SID = sid;

        GA_PACK_LE_3_BYTE_VAL
        (
            bc_src_db[rem_src_db_count].broadcast_ID,
            bc_id
        );
...
void appl_ga_bc_sd_snk_clear_rem_src_db(void)
{
    ...
    for (index = 0U; index < rem_src_db_count; index++)
    {
        ...
        (
            &bc_src_db[0U],
            0U,
            APPL_GA_BC_SD_SNK_REM_SRC_DB_COUNT
        );
    }
    rem_src_db_count = 0U;
}
        ...
        (rem_src_db_count < APPL_GA_BC_SD_SNK_REM_SRC_DB_COUNT))
    {
        for (index = 0U; index < rem_src_db_count; index++)
        {
            if (GA_COMPARE_ENDPOINT_ADDR
               (
                   &addr[0U],
                   &bc_src_db[index].adv_addr[0U]
               ))
            {
                if (GA_COMPARE_ENDPOINT_TYPE(addr_type, bc_src_db[index].adv_addr_type))
                {
                    if (NULL != db_index)
                    {
                        *db_index = index;
...
void appl_ga_bc_sd_get_sg_bis_info_index_and_setup_iso(void)
{
    ...
    /*
     * Fetch the valid SG Index and BIS Index to Setup ISO
     * Data Path.
     */
    if (GA_SUCCESS ==
        appl_ga_bc_sd_snk_fetch_sg_bis_info_index_from_bis_index
        (
            snk_param->bis_index_enable[snk_param->bis_index_curr_iso_enable_disable],
            &sg_info_index,
            &bis_info_index
        ))
    {
        ...
        {
            /* Setup ISO itself failed, break */
            APPL_TRC("Setup ISO Failed !");
        }
        else
        {
            /* Setup ISO is Ongoing, Wait */
            snk_param->bis_index_curr_iso_enable_disable++;
        }
    }
...
void appl_ga_bc_sd_get_sg_bis_info_index_and_remove_iso(void)
{
    ...
    /*
     * Fetch the valid SG Index and BIS Index to Remove ISO Data by referring
     * to first BIS index in the bis_index_enable[]
     */
    if (GA_SUCCESS ==
        appl_ga_bc_sd_snk_fetch_sg_bis_info_index_from_bis_index
        (
        ...
        ))
    {
        ...
        {
            /* Removal of ISO itself failed, break */
     ...
     )
{
    ...
    APPL_TRC("BIS Role: Sink\n");
    APPL_TRC
    (
        "BIS Connection Handle: 0x%04X\n",
        snk_param->bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_handle
    );
    APPL_TRC
    (
