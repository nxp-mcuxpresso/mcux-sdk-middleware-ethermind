...
LC3_ENCODER_CONFIG lc3_enc_config;
LC3_DECODER_CONFIG lc3_dec_config;
LC3_ENCODER_CHANNEL_CNTX lc3_enc_ctx[APPL_LC3_CHANNELS_MAX];
LC3_DECODER_CHANNEL_CNTX lc3_dec_ctx[APPL_LC3_CHANNELS_MAX];
...
API_RESULT appl_ga_hci_event_callback(UINT8 event_code, UINT8 * event_data, UINT8 event_datalen)
{
    ...
    case HCI_LE_META_EVENT:
        ...
        case HCI_LE_CIS_REQUEST_SUBEVENT:
            ...
            {
                CONSOLE_OUT("No registered ASEs connected with this ACL: 0x%04X, CIG: 0x%04X, CIS: 0x%04x\n", acl_conn_handle, cig_id, cis_id);
            }
            else
            {
        ...
        case HCI_LE_CIS_ESTABLISHED_SUBEVENT:
            ...
            if (0 == status)
            {
                ...
                if (APPL_GA_GATT_SERVER == curr_GATT_role)
                {
                    ...
                    {
                        CONSOLE_OUT("No registered ASEs connected with this CIS 0x%04x", cis_conn_handle);
            ...
            {
                CONSOLE_OUT("CIS Established Failed: 0x%02X!!!\n", status);
            }
            break;
    ...
    case HCI_COMMAND_COMPLETE_EVENT:
        CONSOLE_OUT("Received HCI_COMMAND_COMPLETE_EVENT.\n");
            ...
            case HCI_LE_SETUP_ISO_DATA_PATH_OPCODE:
                if (0x00 == status)
                {
                    ...
                    if (APPL_GA_GATT_SERVER == curr_GATT_role)
                    {
                        ...
                        if (GA_FAILURE == appl_ga_fetch_local_ase_db_index_from_cis_conn_handle(cis_conn_handle, local_ase_db_index_array, &ase_count))
                        {
                            CONSOLE_OUT("No registered ASEs connected with this CIS 0x%04x", acl_conn_handle);
                        }
                        else
                        {
    ...
    case HCI_DISCONNECTION_COMPLETE_EVENT:
        /* Status */
        hci_unpack_1_byte_param(&status, event_data);
        CONSOLE_OUT("\tStatus: 0x%02X\n", status);
        event_data += 1;

        /* Connection Handle - CIS or ACL */
        hci_unpack_2_byte_param(&value_2, event_data);
        CONSOLE_OUT("\tConnection Handle: 0x%04X\n", value_2);
        event_data += 2;
        ...
        if (0x00 == status)
        {
            if (APPL_GA_GATT_SERVER == curr_GATT_role)
            {
                ...
                {
                    CONSOLE_OUT("No registered ASEs connected with this handle 0x%04x", value_2);
                }
          ...
          )
{
    ...
    if (NULL == device)
    {
        CONSOLE_OUT("NULL Device Unexpected!\n");
        return GA_FAILURE;
    }
    ...
    case GA_GET_CAPABILITIES_CNF:
        ...
        else if (GA_CAP_TYPE_LOCATIONS == capab->type)
        {
            ...
            {
                CONSOLE_OUT("\tIncorrect Length!!!\n");
                CONSOLE_OUT("\tLength - 0x%02X\n", capab->datalen);
                CONSOLE_OUT("\tData:\n");
        ...
        else if (GA_CAP_TYPE_AVAIL_CONTEXTS == capab->type)
        {
            if (0x04 == capab->datalen)
            {
                ...
                {
                    CONSOLE_OUT("\tServer is not available to receive audio for any Context Type value.\n");
                }
                ...
                {
                    CONSOLE_OUT("\tServer is not available to transmit audio for any Context Type value.\n");
                }
#ifdef APPL_GA_CHAIN_CLIENT_PROCEDURES
        ...
        {
            CONSOLE_OUT("Unknown Capability Type!!!\n");
        }
        break;
...
#ifdef INCLUDE_ETHERMIND_LC3
                            /* De-init LC3 Module */
                            GA_mem_set(&lc3_enc_ctx,0, sizeof(lc3_enc_ctx));
...
#ifdef INCLUDE_ETHERMIND_LC3
                            /* De-init LC3 Module */
                            GA_mem_set(&lc3_dec_ctx,0, sizeof(lc3_dec_ctx));
    ...
    default:
        CONSOLE_OUT("Received UnHandled Event\n");
        break;
    }
    ...
    case GA_BC_ENABLE_CNF:
            ...
            GA_broadcast_decode_event_params(GA_BC_ENABLE_CNF, ga_data, &bc_enable))
        {
            ...
            if (0 < bc_enable.num_bis)
            {
                ...
                CONSOLE_OUT("Dissociate Broadcast Sink...\n");
                ...
                CONSOLE_OUT("Release Broadcast Sink...\n");
...
GA_RESULT appl_ga_lc3_decode_n_play(UCHAR* data, UINT16 datalen)
{
    ...
    for (uint8_t chan_ind = 0; chan_ind < lc3_dec_config.n_c; chan_ind++)
    {
        retval = LC3_decode_a_frame
					(
						&lc3_dec_ctx[chan_ind],
						&enc_buffer[chan_ind][0],
						&dec_buffer[chan_ind][0],
						flg_bytes[0],
						dec_work_buffer
					);
        if (LC3_DECODER_SUCCESS == retval)
        {
...
int main_ga_legacy_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 2:
            ...
            {
#if ((defined APPL_GA_SAMPLE_APP_UCC) || (defined APPL_GA_SAMPLE_APP_UCS))
                appl_ga_init_legacy();
#else /* ((defined APPL_GA_SAMPLE_APP_UCC) || (defined APPL_GA_SAMPLE_APP_UCS)) */
                appl_ga_register();
#endif /* ((defined APPL_GA_SAMPLE_APP_UCC) || (defined APPL_GA_SAMPLE_APP_UCS)) */
            }
        ...
        case 9:
            ...
            for (index = 0; index < ase_count; index++)
            {
                ...
                CONSOLE_OUT("Enter the Contexts: ");
                CONSOLE_IN("%d", &choice);
        ...
        case 31:
            ...
            CONSOLE_OUT("Receiver Start Ready on SEP\n");
        ...
        case 32:
            ...
            CONSOLE_OUT("Update Codec on SEP\n");
        ...
        case 33:
            ...
            CONSOLE_OUT("Suspend Codec on SEP\n");
        ...
        case 34:
            ...
            CONSOLE_OUT("Receiver Stop Ready on SEP\n");
        ...
        case 35:
            ...
            CONSOLE_OUT("Release Codec on SEP\n");
      ...
      /* Temp */
...
#ifdef BAP_BROADCAST
        case 80:
            CONSOLE_OUT ("Allocating Broadcast Session...\n");
        ...
        case 81:
            CONSOLE_OUT ("Freeing Broadcast Session...\n");
        ...
        case 82:
            ...
            CONSOLE_OUT ("Configuring Broadcast Session...\n");
        ...
        case 83:
            CONSOLE_OUT ("Registering SEP to Broadcast Session...\n");
...
#ifdef GA_PBP
        {
            ...
            CONSOLE_OUT("Registering Advset to Session...\n");
            ...
            CONSOLE_OUT ("Setup Broadcast Session...\n");
        ...
        case 85:
            CONSOLE_OUT ("Release Broadcast Session...\n");
            retval = GA_broadcast_end_announcement
                     (
                         appl_bcast_ssn
                     );
        ...
        case 86:
            CONSOLE_OUT ("Start Broadcast Session...\n");
        ...
        case 87:
            ...
            CONSOLE_OUT ("Suspend Broadcast Session...\n");
        ...
        case 90:
            ...
            CONSOLE_OUT ("Setup Broadcast Sink...\n");
        ...
        case 92:
            ...
            CONSOLE_OUT ("Associate Broadcast Sink...\n");
        ...
        case 94:
            ...
            CONSOLE_OUT ("Enable Broadcast Sink...\n");
        ...
        case 95:
            CONSOLE_OUT ("Disable Broadcast Sink...\n");
        ...
        case 101:
            ...
            CONSOLE_OUT("GA_update_ntf_configuration...\n");
            ...
            CONSOLE_OUT("BAP_SUPPORT_CONFIG_SELECTION NOT defined\n");
        ...
        case 105:
            ...
            CONSOLE_OUT ("BAP_SUPPORT_CONTEXT_MANAGE NOT defined\n");
#endif /* BAP_SUPPORT_CONTEXT_MANAGE */
            break;
    ...
    else
    {
        ...
        if (APPL_GA_ISO_TX == data_path_direction)
        {
            ...
            */
            ...
            lc3_retval = lc3_ndsp_enc_set_config_params
    					(
    						&lc3_enc_config,
							APPL_GA_CODEC_SUPP_SAMPL_FREQ,					//the sampling rate
							APPL_GA_ASE_CHANNEL_COUNT,						//number of channels.  If CIS/BIS, = 1
							APPL_GA_CODEC_SUPP_FRAME_DURATION_MS,			//frame period
							APPL_GA_SAMPLE_SIZE_IN_BITS						//the compressed data rate
    					);

    		if (lc3_retval == LC3_ENCODER_SUCCESS)
    		{
    			for (uint8_t chan_id = 0; (!lc3_retval) && (chan_id < APPL_GA_ASE_CHANNEL_COUNT); chan_id++)
    			{
    				//inititialize the ch cnxt
    				lc3_retval = lc3_enc_init_ch_cntx
    							(
    								&lc3_enc_ctx[chan_id],
    								&lc3_enc_config,
									target_enc_bytes[chan_id]
    							);
    			}
    		}
        ...
        else if (APPL_GA_ISO_RX == data_path_direction)
        {
            ...
            */
            ...
            lc3_retval = lc3_ndsp_dec_set_config_params
            			 (
            				 &lc3_dec_config,
							 APPL_GA_CODEC_SUPP_SAMPL_FREQ,
							 APPL_GA_ASE_CHANNEL_COUNT,
            ...
            {
                for (uint8_t chan_index = 0; (!lc3_retval) && (chan_index < APPL_GA_ASE_CHANNEL_COUNT); chan_index++)
                {
                	lc3_retval = lc3_dec_init_ch_cntx
                				(
                					&lc3_dec_ctx[chan_index],
    								&lc3_dec_config,
									lc3_dec_config.n_f_len  //TODO: Verify this param
    							);
                }
            }
          ...
          )
{
    ...
    {
    	for (uint8_t chan_in = 0; chan_in < lc3_enc_config.n_c; chan_in++)
    	{
        	lc3_encoded_bytes = LC3_encode_a_frame
        						(
        							&lc3_enc_ctx[chan_in],
        							&pcm_buffer[chan_in][0],
    								&enc_buffer[chan_in][0],
									enc_work_buffer
    							);
...
void appl_ga_register(void)
{
    ...
    CONSOLE_OUT("Set ISOC Host feature support - 0x%04X\n", retval);
    ...
    /* Register VOCS */
    CONSOLE_OUT("GA_vc_register. for VOCS \n");
    ...
    /* Register VCP-AICS */
    CONSOLE_OUT("GA_vc_register..for AICS \n");
    retval = appl_vcp_rd_reg_opt_service
             (
    ...
    /* Register MCP-AICS */
    CONSOLE_OUT("GA_mc_register_aics \n");
...
void appl_ga_automate_register_pacs(void)
{
    ...
    for (index = 0; index < count; index++)
    {
        if (GA_ROLE_SINK == role)
        {
            ...
            CONSOLE_OUT("Enter Supported Contexts (0x):  Prohibited: 0, Media: 4\n");
            CONSOLE_IN("%x", &choice);
...
#else /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
            CONSOLE_OUT("Enter Available Contexts (0x):  Prohibited: 0, Media: 4\n");
            CONSOLE_IN("%x", &choice);
        ...
        CONSOLE_OUT("[PACS]GA capability ID: 0x%02X\n", ga_local_audio_cap_count);
    }
    ...
    CONSOLE_OUT("[PACS]Registering GA capability: Retval: 0x%04X\n", retval);
    for (index = 0; index < count; index++)
    {
        CONSOLE_OUT("Registered ID : 0x%02X\n", reg_id[index]);
    }
    ...
    if (GA_ROLE_SINK == role)
    {
        ...
        CONSOLE_OUT("[PACS]Registering GA Audio Contexts and Locations: 0x%04X\n", retval);
    }
    else if (GA_ROLE_SOURCE == role)
    {
        retval = GA_register_audio_contexts_and_locations
        ...
        CONSOLE_OUT("[PACS]Registering GA Audio Contexts and Locations: PACS Record 0x%04X\n", retval);
    }
...
void appl_ga_register_ascs(void)
{
     ...
     */
    CONSOLE_OUT("[ASCS]Registering ASE\n");
    ...
    {
        CONSOLE_OUT("Invalid Num of ASE Entered!!!\n");
    }
    else if (ase_count < 1)
    {
        CONSOLE_OUT("Register minimum 1 ASE[1 Src or 1 Sink]\n");
    }
    else
    {
        ...
        for (index = 0; index < ase_count; index++)
        {
...
#else /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
            CONSOLE_OUT("Enter ASE Role: (1 - Sink, 2 - Source):");
            CONSOLE_IN("%d", &choice);
            ...
            cap_index = 1U;
#else /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
            CONSOLE_OUT("Enter the Codec Capability Index:");
            CONSOLE_IN("%d", &choice);
            ...
            {
                CONSOLE_OUT("Capability Index should be >0 and <APPL_GA_MAX_AUDIO_CAPABILITIES\n");
            ...
            if (GA_ROLE_SINK == local_ase_codec_conf.role)
            {
                ...
                CONSOLE_OUT("[ASCS]Registered ASE ID, Retval - 0x%04X\n", retval);

                if (GA_SUCCESS == retval)
                {
                    CONSOLE_OUT("Sink ASE ID %d\n", ase_id);
                }
            }
...
void appl_ga_unicast_set_remote_dev(void)
{
    ...
    {
        CONSOLE_OUT("Device registered\n");
    }
    else
    {
        CONSOLE_OUT("Maximum Simultaneous connections reached\n");
    }
...
void appl_ga_unicast_client_setup(void)
{
    ...
    CONSOLE_OUT("Setup Endpoint\n");
    CONSOLE_OUT("Setup PACS: Audio Role Discovery - PACS Discovery\n");
    retval = GA_setup(&appl_ga_device, &appl_ga_context);
    CONSOLE_OUT("Retval - 0x%04X\n", retval);
    CONSOLE_OUT("Context: %d\n", appl_ga_context);
...
void appl_ga_unicast_client_get_pacs_capabilities(void)
{
    ...
    CONSOLE_OUT("Get PACS Capabilities - Read Audio Capability, Location, Supp/Avail Contexts\n");
...
void appl_ga_unicast_client_discover_ascs(void)
{
    ...
    CONSOLE_OUT("Discover ASEs in Endpoint - ASCS Discovery\n");
...
void appl_ga_unicast_client_configure_codec(void)
{
    ...
    for (index = 0; index < peer_ase_count; index++)
    {
        ...
        CONSOLE_OUT("Enter the Capability Set Index: ");
        CONSOLE_IN("%d", &choice);
#endif /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
        /*
    ...
    CONSOLE_OUT("Configure Codec on ASE\n");
...
void appl_ga_unicast_client_setup_cig(void)
{
    ...
    for (index = 0; index < cis_count; index++)
    {
        ...
        ase_id = 1U;
#else /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
        CONSOLE_OUT("CIS ID to the same device and with the same Conf\n");
        CONSOLE_OUT("Enter ASE ID: ");
        CONSOLE_IN("%d", &choice);
        ase_id = (UINT8)choice;
#endif /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
    ...
    CONSOLE_OUT("Create CIG Parameters: \n");
...
void appl_ga_unicast_client_configure_qos(void)
{
    ...
    CONSOLE_OUT("Setup Codec on ASE: QoS Configuration\n");
...
void appl_ga_unicast_client_enable(void)
{
    ...
    for (index = 0; index < peer_ase_count; index++)
    {
        ...
        else
        {
            ...
            CONSOLE_OUT("Enter Context Type: ");
            CONSOLE_IN("%d", &choice);
            content_info[index].contexts = (UINT16)choice;
#endif /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
...
void appl_ga_read_config_file(void)
{
    ...
    if (ret < 0)
    {
        CONSOLE_OUT("*** FAILED to Read Configuration for ENABLE_AUDIO_DUMP: %d\n",
            ret);
    }
    else
    {
        CONSOLE_OUT("Enable Audio Dump = '%s'\n", config_buf);
...
#else /* APPL_GA_USE_CONFIG_FILE_FOR_INPUT */
    CONSOLE_OUT("Enter Advertising Enable (1 - Enable, 0 - Disable): ");
    CONSOLE_IN("%d", &choice);
