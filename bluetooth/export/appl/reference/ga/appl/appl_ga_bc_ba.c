...
/* Define this for few PTS test cases. Disabled by default */
/* #define APPL_BCSRC_HAVE_DYNAMIC_BCASTCODE_INPUT */
...
================ Device Management ~ Configure ============ \n\
    ...
    8. Update Address and Type for Broadcast Source \n\
    9. Configure Eadv Parameters \n\
\n\
...
void appl_ga_bc_ba_get_input_for_add_src_bis(UINT8 index, UINT8 sg_index);
...
void appl_ga_bc_ba_src_display_bis_stream_info
     (
         UINT8  sg_info_index,
         UINT8  bis_info_index
     );
          ...
          )
{
    ...
    case BASS_CE_RELEASE_CNF:
        ...
        if (GA_SUCCESS == evt_status)
        {
            ...
            appl_bc_bass_rem_sd_db[appl_bc_rem_sd_in_use].bc_bass_rx_state_count = 0U;
        }
        break;
            ...
            else
            {
                    ...
                    appl_bc_bass_rem_sd_db[sd_index].bc_rx_state_db[rx_state_db_index].bc_rx_state.info.big_encryption)
                {
                    ...
                    (
                        ...
                        "Decryption.\nSource_ID: 0x%02X. Use Menu Option 24\n",
                              ...
                              ))
            {
                                   ...
                                   )))
                {
                    ...
                    if (GA_SUCCESS == appl_bc_bass_ba_fetch_free_bc_src_index_in_db(&i))
                    {
                        ...
                        APPL_TRC("Source added in Index: %d\n", i);
                    }
                    else
                    {
                        /* Reached Max */
...
void main_ba_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        switch(menu_choice)
        {
            ...
            case 2:
                ...
                else
                {
                    ...
                    if (GA_SUCCESS == retval)
                    {
                        CONSOLE_OUT
                        (
                            "Is BA collocated Broadcast Source required?\n1.Yes\n2.No\n"
                        );
                        CONSOLE_IN("%d", &choice);
                        if (1U == (UINT8)choice)
                        {
                            /* Assign the role as BA + BC Src after initialization */
                            bc_role = BC_ROLE_BA | BC_ROLE_BC_SRC;

                            /*
                             * For BC BA Collocated BC Src, update the
                             * connection interval.
                             * Connection interval is updated here to be used
                             * when connection is performed from GA Menu.
                             */
            ...
            case 3:
                if (BC_ROLE_BA != (BC_ROLE_BA & bc_role))
                {
                    APPL_TRC("BA role is not registered for the device. "
            ...
            case 8:
                ...
                {
                    /*
                     * If the addr_type is public, then get the
                     * local BD_Address
                     */
                    device.bd_type = BD_ADDR_TYPE_PUBLIC;

                    BT_hci_get_local_bd_addr(&device.bd_addr[0U]);
                }
                else if (2U == (UINT8)choice)
                {
                    /*
                     * If the addr_type is random, then take input for
                     * Random Address
                     */
                    ...
                    (2U == (UINT8)choice))
                {
                    ...
                    /*
                     * Update the Address with the GA BRR for BC Src.
                     */
                    appl_ga_utils_config_brr_eadv();
                }
                break;
            ...
            case 12:
                ...
                if (IS_APPL_GA_BC_ROLE_BA_PLUS_BC_SRC(bc_role))
                {
                    /* For BC BA Collocated BC Src, update the connection interval */
                    local_le_ext_conn_params.conn_interval_min[0U] = 0x0190U;
                    local_le_ext_conn_params.conn_interval_max[0U] = 0x0190U;
                }
                ...
                local_le_ext_conn_params.own_addr_type = own_addr_info.bd_type;
            ...
            case 14:
                /*
                 * If a device is already connected by other profile clients,
                 * need to update/add the device here in the DB.
                 * Eg: In PTS, the Adv report doesn't include BASS Service UUID
                 * for few TCs.
                 */
                /*
                 * TODO: If a device advertises BASS Service UUID during scan
                 * by any module, automatically add the device into
                 * appl_bc_bass_rem_sd_db DB.
                 */
                CONSOLE_OUT("Does the device support BASS Service?\n");
                CONSOLE_OUT("1. Yes\n2. No\n");
                CONSOLE_IN("%d", &choice);
                if (1U == (UINT8)choice)
                {
                                      ...
                                      ))
                    {
                        ...
                        {
                            APPL_ERR("Device already in DB !\n");
                        }
                    }
                }
                else
                {
                    APPL_TRC("Device doesn't support BASS Service !\n");
                }
                break;
            ...
            case 40:
                ...
                if (BC_ROLE_BC_SRC == (BC_ROLE_BC_SRC & bc_role))
                {
                    APPL_TRC("Call Free Session\n");
                    break;
                }
                ...
                if (GA_SUCCESS == retval)
                {
                    /* For BA Collocated Source, this is already taken care during Init */
            ...
            case 42:
                ...
                {
#ifdef APPL_BCSRC_HAVE_DYNAMIC_BCASTCODE_INPUT
                    CONSOLE_OUT("Enter Broadcast Code? (1/0): ");
                    CONSOLE_IN("%d", &choice);

                    if (1 == choice)
                    {
                        CONSOLE_OUT("Enter 16 octet Broadcast Code (LSB0 LSB1 ... MSB): ");
                        bc_code_ptr = &appl_bc_src_params.bc_code[0U];
                        appl_input_octets(bc_code_ptr, 16);
                    }
                    else
#endif /* APPL_BCSRC_HAVE_DYNAMIC_BCASTCODE_INPUT */
                    {
                        /*
            ...
            case 44:
            {
...
#ifdef APPL_GA_USE_CONFIG_FILE_FOR_INPUT
                appl_bc_src_params.advset_handle = appl_ga_utils_bap_get_adv_set_identifier();
                ...
                retval = GA_broadcast_register_advset_info
                         (
                             appl_bc_src_params.bc_ssn,
                             appl_bc_src_params.advset_handle,
                             sdata,
                             sdatalen,
                             adata,
                             adatalen
            ...
            case 50:
                ...
                for (j = 0U; j < appl_bc_src_params.bc_subgroup_info[0].bc_nstream; j++)
                {
                    GA_mem_copy
                    (
                        &cs_conf_ltv[j],
                ...
                retval = GA_broadcast_update_sep
                         (
                             appl_bc_src_params.bc_subgroup_info[index].bc_sep,
                             appl_bc_src_params.bc_ssn,
                             &appl_bc_src_params.bc_subgroup_info[index].bc_basic_announcement_1.codec,
            ...
            case 90:
                ...
                APPL_ERR("BASS_SUPPORT_CONFIG_SELECTION NOT defined !\n");
...
GA_RESULT appl_bc_bass_ba_add_bc_src(UINT8 index)
{
    ...
    {
        /* Update BIS Sync field of SG as per user input */
        appl_ga_bc_ba_get_input_for_add_src_bis(index, i);
        GA_mem_copy
        (
            &write_pdu[marker],
...
void appl_ga_bc_ba_get_input_for_add_src_bis(UINT8 index, UINT8 sg_index)
{
    ...
    if (BASS_BIS_SYNC_DO_NOT_SYNC_TO_BIS_INDEX == appl_bc_bass_ba_bc_src[index].appl_bc_bass_bc_src.src.pa_sync)
    {
        /*
         * Update BIS_Sync to BASS_BIS_SYNC_DO_NOT_SYNC_TO_BIS_INDEX if PA_Sync
         * is BASS_BIS_SYNC_DO_NOT_SYNC_TO_BIS_INDEX
         */
        appl_bc_bass_ba_bc_src[index].appl_bc_bass_bc_src.src.bis_sync[sg_index] =
            BASS_BIS_SYNC_DO_NOT_SYNC_TO_BIS_INDEX;
    }
    else
    {
        APPL_TRC
        ...
        CONSOLE_OUT("1. Do not Sync to BIS(s)\n2. Sync to BIS(s)\n3. No preference\n");
        CONSOLE_IN("%d", &choice);
        if (1U == (UINT8)choice)
        {
            appl_bc_bass_ba_bc_src[index].appl_bc_bass_bc_src.src.bis_sync[sg_index] =
        ...
        else if (2U == (UINT8)choice)
        {
            ...
            for (i = 0U; i < num_bis; i++)
            {
                CONSOLE_OUT("Enter the BIS index: ");
                CONSOLE_IN("%d", &choice);
                bis_index = (UINT8)choice;
                BASS_BIS_SYNC_STATE_UPDATE_SYNC_TO_BIS_INDEX(bis_sync, bis_index);
            }
...
GA_RESULT appl_bc_ba_send_pa_sync_info(void)
{
    ...
    /* Octet-1 of Service_Data */
    /* Source_ID */
    CONSOLE_OUT("Enter the Source ID received as part of BC Rx State Notification for Add Source\n");
    CONSOLE_IN("%d", &src_id);
    svc_data |= (src_id << 8U);
...
void appl_bc_ba_add_local_bc_src_to_src_db(void)
{
    ...
    /*
     * Fetch own_addr_info for broadcast to check for public or random address,
     * to add it to DB
     */
    appl_ga_utils_fetch_own_address_and_type
    (
    ...
    bc_source_announcement.address_type = bc_src_addr_info.bd_type;
    bc_source_announcement.address = &bc_src_addr_info.bd_addr[0U];
    ...
    APPL_TRC("BD_Address of the Source added to DB: ");
    for (i = 0U; i < GA_BD_ADDR_SIZE; i++)
    {
        APPL_TRC("%02X ", bc_source_announcement.address[i]);
    }
    APPL_TRC("\n");
    APPL_TRC
    (
        "BD_Address_Type of the Source added to DB: 0x%02X\n",
        bc_source_announcement.address_type
    );
}
...
void appl_bc_src_bap_set_bc_params_using_default_values(APPL_GA_ROLE role)
{
    ...
    for (index = 0U; index < appl_bc_src_params.bc_num_subgroups; index++)
    {
        ...
        for (i = 0U; i < appl_bc_src_params.bc_subgroup_info[index].bc_nstream; i++)
        {
            ...
            if (max_cc < appl_ga_utils_fetch_sacc_from_aca(cs_conf.aca))
            {
                max_cc = appl_ga_utils_fetch_sacc_from_aca(cs_conf.aca);
            }
    ...
    appl_bc_src_params.bc_qos_conf.max_sdu *= max_cc;
...
UINT8 appl_bc_bass_ba_is_pacs_compatible(UINT8 src_index)
{
                      ...
                      ))
    {
                          ...
                          ))
        {
            ...
            /* Validate PACS of SD with the Codec Spec Conf of the received BC Source */
            for (index = 0U; index < rem_pacs_db.pacs_snk_cap_count; index++)
            {
                capabilitySupp = GA_TRUE;

                for (i = 0U; i < rem_pacs_db.pacs_snk_cap[index].cap_count_per_instance; i++)
                {
                    appl_ga_utils_bap_parse_and_display_codec_spec_cap_ltv
                    (
                        rem_pacs_db.pacs_snk_cap[index].cap[i].codec.ie,
                        rem_pacs_db.pacs_snk_cap[index].cap[i].codec.ie_len,
                        &cs_cap
                    );
        ...
        (AUDIO_ISO_IDLE == appl_bc_src_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].iso_state))
    {
        ...
        else
        {
            ...
            retval = appl_ga_utils_audio_src_le_setup_iso_data_path
                     (
                         appl_bc_src_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].bis_handle,
                         &appl_bc_src_params.bc_subgroup_info[sg_info_index].bis_info[bis_info_index].stream_symbol
                     );
            if (GA_SUCCESS == retval)
            {
