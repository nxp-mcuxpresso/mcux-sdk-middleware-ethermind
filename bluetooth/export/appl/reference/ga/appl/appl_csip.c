...
#if ((defined APPL_CSIP_SETMEMBER) || (defined APPL_CSIP_SETCOORDINATOR))
...
DECL_STATIC UCHAR   appl_csip_encrypt_req = GA_FALSE;
...
void appl_csip_decsirk_callback(UCHAR * data, UINT16 datalen);
void appl_csip_process_sirk(GA_ENDPOINT * device, UCHAR type, UCHAR * sirk);
GA_RESULT appl_csip_is_sirk_valid(UCHAR * sirk);
          ...
          )
{
    ...
    if ((NULL != ga_data) && (0U != ga_datalen))
    {
        ...
        ga_data = csip_event->data;
        ga_datalen = csip_event->datalen;
    }
    ...
    case GA_CS_SIRK_NTF:
        {
            ...
            if ((CSIP_SIRK_LEN + 1U) == ga_datalen)
            {
                ...
                /* TODO: Device is obtained from the Global Device, get the device */
                appl_csip_process_sirk(&csip_device, sirk->type, sirk->value);
            }
        }
        break;
...
int appl_csip_sc_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 4:
            retval = GA_cs_discover(&csip_device, NULL);
            APPL_TRC("Retval - ");
        ...
        case 7:
            CONSOLE_OUT("Enter the CSIP Handle: ");
            CONSOLE_IN("%d", &choice);
            if (CSIP_MAX_CONTEXTS <= (UINT8)choice)
            {
                APPL_INF("Invalid handle !\n");
            }
            else
            {
                appl_csip_context = (UINT8)choice;
            }
            break;
...
void appl_csip_sm_sirk_callback(UCHAR * data, UINT16 datalen)
{
    ...
    if (appl_csip_encrypt_req)
    {
        ...
        GA_mem_copy(&appl_csip_sirk.value[0U], data, CSIP_SIRK_LEN);
    ...
    else
    {
        ...
        appl_csip_sirk.type = CSIP_SIRK_TYPE_PLAIN_TEXT;
        GA_mem_copy(&appl_csip_sirk.value[0U], data, CSIP_SIRK_LEN);
        appl_dump_bytes(data, CSIP_SIRK_LEN);
        APPL_TRC("============================\n");
    }
...
int appl_csip_sm_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 26:
            ...
            if ((GA_TRUE == appl_csip_encrypt_req) &&
                (CSIP_SIRK_TYPE_PLAIN_TEXT == appl_csip_sirk.type))
            {
                if (!(GA_IS_NON_ZERO_ENDPOINT_ADDR(csip_device.bd_addr)))
                {
                    APPL_ERR("Connection required\n !");
                }
                else
                {
                    appl_csip_sirk.type = CSIP_SIRK_TYPE_ENCRYPTED;
                    appl_csip_encrypt_sirk(&csip_device);
                }
            }
            else if ((GA_FALSE == appl_csip_encrypt_req) &&
                     (CSIP_SIRK_TYPE_ENCRYPTED == appl_csip_sirk.type))
            {
                if (GA_SUCCESS == appl_ga_utils_get_conn_device_index(csip_device, &conn_dev_index))
                {
                    appl_ga_utils_fetch_key_of_conn_dev(conn_dev_index, key);
                    APPL_TRC("Generating Decrypted SIRK for the bonded device...\n");
        ...
        case 30:
            CONSOLE_OUT
            (
                "Enter the 16 octet Key to Encrypt SIRK (Ex: 01 A3 ...): "
            );
...
void appl_csip_process_sirk(GA_ENDPOINT * device, UCHAR type, UCHAR * sirk)
{
    ...
    if (CSIP_SIRK_TYPE_ENCRYPTED == type)
    {
        if (GA_SUCCESS == appl_ga_utils_get_conn_device_index(*device, &conn_dev_index))
        {
            ...
            retval = CSIP_sirk_process(sirk, key, 0x00, appl_csip_decsirk_callback);
            APPL_TRC("Retval - 0x%04X\n", retval);
        }
    }
...
void appl_csip_encrypt_sirk(GA_ENDPOINT * device)
{
    ...
    if ((CSIP_SIRK_TYPE_ENCRYPTED == appl_csip_sirk.type) &&
        (GA_SUCCESS == appl_csip_is_sirk_valid(appl_csip_sirk.value)))
    {
        if (GA_SUCCESS == appl_ga_utils_get_conn_device_index(*device, &conn_dev_index))
        {
            appl_ga_utils_fetch_key_of_conn_dev(conn_dev_index, key);
...
GA_RESULT appl_csip_is_sirk_valid(UCHAR * sirk)
{
    ...
    for (index = 0U; index < CSIP_SIRK_LEN; index++)
    {
        if (0U != sirk[index])
        {
            retval = GA_SUCCESS;
...
void appl_ga_csip_sc_init()
{
    ...
    APPL_TRC("APPL_CSIP_SETCOORDINATOR not defined!");
#endif /* APPL_CSIP_SETCOORDINATOR */
}

void appl_ga_csip_sc_shutdown()
{
    GA_RESULT retval;
...
void appl_ga_csip_sm_init()
{
    ...
    APPL_TRC("APPL_CSIP_SETMEMBER not defined!");
#endif /* APPL_CSIP_SETMEMBER */
}

void appl_ga_csip_sm_shutdown()
{
    GA_RESULT retval;
...
#endif /* ((defined APPL_CSIP_SETMEMBER) || (defined APPL_CSIP_SETCOORDINATOR)) */
