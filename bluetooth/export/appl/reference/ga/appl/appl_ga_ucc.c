...
UINT8 appl_ga_ucc_get_cig_cis_shared_bidirectional_ase_index
      (
          UINT8   ascs_index,
          UINT8   ase_index,
                        ...
                        if (0x000 != adlength)
                        {
                            ...
                            if (GA_ADTYPE_SERVICE_DATA == adtype)
                            {
                                ...
                                if ((GA_SERVICE_CAS == svc_uuid) ||
                                    ...
                                    (GA_SERVICE_HAS == svc_uuid) ||
                                    (GA_SERVICE_ASCS == svc_uuid) ||
                                    (GA_SERVICE_PACS == svc_uuid))
                                {
        ...
        for (index = 0U; index < ase_count; index++)
        {
                ...
                if (GA_ROLE_SOURCE == local_role)
                {
                                      ...
                                      ))
                    {
                        ...
                        (
                            ...
                            remote_ascs[ascs_index].ascs.ase[ase_index]
                        );
                        GA_FORMAT_DISPLAY();

                        if (GA_ASE_STATE_STREAMING == remote_ascs[ascs_index].ascs.ase[ase_index].state)
                        {
                            /*
                             * It means RX_start_ready(Streaming) is already
                             * received for given ASE before ISO data path was
                             * setup.
                             * So, start transmitting audio data for the same.
                             */
                    ...
                    remote_ascs[ascs_index].ascs.ase[ase_index].iso_state))
            {
                ...
                if (GA_ROLE_SOURCE == local_role)
                {
                    ...
                    {
                        APPL_ERR("[APPL][GA][UCC][%s]: ISO Data Path Removal Status Update entry failed !\n", UC_GET_ROLE_STRING(local_role));
                    }
                }
                else
                {
            ...
            for (j = 0U; j < MAX_REM_ASCS_INFO; j++)
            {
                for (k = 0U; k < remote_ascs[j].ascs.ase_count; k++)
                {
                        ...
                        (CIS_STATE_IDLE == remote_ascs[j].ascs.ase[k].cig_cis_info.cis_state))
                    {
                        ...
                        if (GA_TRUE == appl_ga_ucc_get_cig_cis_shared_bidirectional_ase_index
                                       (
    ...
    if (GA_SUCCESS == conn_status)
    {
                          ...
                          ))
        {
            for (index = 0U; index < ase_count; index++)
            {
                 ...
                 * If Connection_Handle identifies a CIS on the Central, the
                 * handle and the associated data paths of that CIS shall
                 * remain valid (irrespective of whether the disconnection was
                 * successful or not).
                 */
                ...
                {
                    if (remote_ascs[ascs_index].ascs.ase_id_active_playback ==
                        remote_ascs[ascs_index].ascs.ase[ase_index[index]].id)
                    {
                        /*
    ...
    if (GA_SUCCESS == retval)
    {
        ...
        for (index = 0U; index < ase_count; index++)
        {
            ...
            if (AUDIO_ISO_SETUP_COMPLETE ==
                remote_ascs[ascs_index].ascs.ase[ase_index[index]].iso_state)
            {
                GA_UCC_GET_LOCAL_ROLE_FOR_ASE
                (
                    remote_ascs[ascs_index].ascs.ase[ase_index[index]].role,
                ...
                if ((GA_ROLE_SOURCE == local_role) &&
                    ...
                    remote_ascs[ascs_index].ascs.ase[ase_index[index]].state))
                {
                    /* We are Audio Source, Start Transmitting Audio data */
                    appl_ga_ucc_start_transmitting_audio
                    (
                        remote_ascs[ascs_index].ascs.ase[ase_index[index]].cig_cis_info.handle,
                        remote_ascs[ascs_index].ascs.ase[ase_index[index]].cs_conf,
                        remote_ascs[ascs_index].ascs.ase[ase_index[index]].qos_conf_set_id,
                        AUDIO_NUM_OF_BITS_PER_SAMPLE
                    );
          ...
          )
{
    ...
    if ((NULL == ga_data) || (0U == ga_datalen) || (GA_FAILURE == ga_status))
    {
    ...
    case GA_SEP_SETUP_CNF:
        ...
        if (GA_SUCCESS == ga_status)
        {
            ...
            else
            {
                ...
                GA_mem_copy(&qos_data_obj.qos_config, &remote_ascs[ascs_index].ascs.ase[ase_index].qos_conf, sizeof(GA_QOS_CONF));
#endif /*defined(LE_AUDIO_SINK_SYNC_ENABLE) && (LE_AUDIO_SINK_SYNC_ENABLE > 0)*/
        ...
        if (GA_SUCCESS == ga_status)
        {
            ...
            if (ase_index < remote_ascs[ascs_index].ascs.ase_count)
            {
                    ...
                    (GA_ASE_STATE_ENABLING == remote_ascs[ascs_index].ascs.ase[ase_index].state))
                {
                    ...
                    {
                        if (remote_ascs[ascs_index].ascs.ase[ase_index].id ==
                            remote_ascs[ascs_index].ascs.ase_id_active_playback)
                        {
...
void appl_ga_ucc_ascs_config_codec(void)
{
    ...
    if (GA_SUCCESS == retval)
    {
        for (index = 0U; index < ase_count; index++)
        {
            ...
            /* TMAP Role: CG, Shall Select as per reqirements */
        ...
        if (GA_SUCCESS == retval)
        {
            ...
            if (ASE_UNIDIRECTIONAL == direction)
            {
                ...
                if (GA_ROLE_SINK ==
                    remote_ascs[ascs_index].ascs.ase[ase_index].role)
                {
                    max_sdu_m_to_s[index] =
                        qos_conf.max_sdu *
                            appl_ga_utils_fetch_sacc_from_aca(remote_ascs[ascs_index].ascs.ase[ase_index].cs_conf.aca);
                    max_sdu_s_to_m[index] = 0U;
                }
                else
                {
                    max_sdu_s_to_m[index] =
                        qos_conf.max_sdu *
                            appl_ga_utils_fetch_sacc_from_aca(remote_ascs[ascs_index].ascs.ase[ase_index].cs_conf.aca);
...
void appl_ga_ucc_ascs_config_qos(void)
{
    ...
    for (index = 0U; index < ase_count; index++)
    {
        ...
        else
        {
            ...
            GA_mem_copy
            (
                &qos_conf[ase_count_valid],
                &remote_ascs[ascs_index].ascs.ase[ase_index].qos_conf,
                sizeof(GA_QOS_CONF)
            );
            qos_conf[ase_count_valid].max_sdu =
                qos_conf[ase_count_valid].max_sdu *
                appl_ga_utils_fetch_sacc_from_aca(remote_ascs[ascs_index].ascs.ase[ase_index].cs_conf.aca);
            ase_count_valid++;
        }
...
void appl_ga_ucc_ascs_enable(void)
{
    ...
    UINT8            ccid[MAX_UC_ASE * MAX_CCID_COUNT] = { 0U };
    ...
    for (index = 0U; index < ase_count; index++)
    {
        ...
        if (ase_index >= remote_ascs[ascs_index].ascs.ase_count)
        {
            APPL_ERR("Unknown ASE ID 0x%02X !\n", ase_id_input);
            /* Skip this ASE ID */
        }
        else
        {
            ...
            else
            {
                ...
                content_info[index].ccid = &ccid[k];
                for (j = 0U; j < content_info[index].num_ccid; j++)
                {
                    ccid[k] = appl_ga_utils_bap_get_ccid();
                    k++;
                }
            }
        }
...
void appl_ga_ucc_receiver_start_ready(void)
{
    ...
    for (index = 0U; index < ase_count; index++)
    {
        ...
        else
        {
            ...
            {
                /*
                 * For Local Role Source, ISO Data Path Setup is not M.
                 * However this is allowed only for Local Role Sink, will be
                 * handled in Core by peer.
                 */
    ...
    if (0U < ase_count)
    {
        ...
        CONSOLE_IN("%d", &choice);
        remote_ascs[ascs_index].ascs.ase_id_active_playback = (UINT8)choice;
    }
...
void appl_ga_ucc_update_metadata(void)
{
    ...
    for (index = 0U; index < ase_count; index++)
    {
        ...
        else
        {
             ...
             */
            /* Fetch the streaming audio context value */
            content_info[ase_count_valid].contexts =
                appl_ga_utils_bap_fetch_streaming_audio_ctx();

            content_info[ase_count_valid].num_ccid = appl_ga_utils_bap_get_ccid_count();

            for (j = 0U; j < content_info[ase_count_valid].num_ccid; j++)
            {
                ccid[j] = appl_ga_utils_bap_get_ccid();
            }
            content_info[ase_count_valid].ccid = ccid;
            ase_count_valid++;
        }
    }
...
void appl_ga_ucc_ascs_release(void)
{
    ...
    for (index = 0U; index < ase_count; index++)
    {
        ...
        else
        {
            ...
            if (GA_TRUE == appl_ga_ucc_get_cig_cis_shared_bidirectional_ase_index
                           (
                               ascs_index, ase_index, &ase_index_using_same_cis
                           ))
            {
                /*
                 * If Bidirectional, then validate if given ASE is in
                 * Enabling/Streaming state
                 */
                ...
                {
                    if (!(((GA_ASE_STATE_ENABLING ==
                        remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].state)
                        ||
                        (GA_ASE_STATE_STREAMING ==
                            remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].state))
                        &&
                        (CIS_STATE_CONNECTED ==
                            remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].cig_cis_info.cis_state)))
                    {
                        ase_id[ase_count_valid] = ase_id_input;
                ...
                else
                {
                    ...
                    (
                        ...
                        "ASE State of ASE ID: 0x%02X is 0x%02X\n",
                        remote_ascs[ascs_index].ascs.ase[ase_index].id,
                        remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].id,
                        remote_ascs[ascs_index].ascs.ase[ase_index].qos_conf.channel.cig_id,
                        remote_ascs[ascs_index].ascs.ase[ase_index].qos_conf.channel.cis_id,
                        remote_ascs[ascs_index].ascs.ase[ase_index].id,
                        remote_ascs[ascs_index].ascs.ase[ase_index].state,
...
void appl_ga_ucc_ase_create_cis(void)
{
    ...
    UINT8      ase_id[MAX_UC_ASE] = { 0 };
    UINT16     acl_handle_array[16U] = { INVALID_CONN_HANDLE };
    UINT16     cis_handle_array[16U] = { INVALID_CONN_HANDLE };
...
void appl_ga_ucc_ase_disconnect_cis(void)
{
                      ...
                      ))
    {
                          ...
                          ))
        {
            ...
            {
                /* Disconnect CIS */
                CONSOLE_OUT("Enter reason for disconnection[In HEX]: ");
                CONSOLE_IN("%x", &input);
                reason = (UCHAR)input;
...
GA_RESULT appl_ga_ucc_is_cis_disconnect_allowed(UINT8 ascs_index, UINT8 ase_index)
{
        ...
        (CIS_STATE_CONNECTED == remote_ascs[ascs_index].ascs.ase[ase_index].cig_cis_info.cis_state))
    {
                       ...
                       ))
        {
             ...
             * and CIS is connected
             */
            if ((AUDIO_ISO_IDLE ==
                    remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].iso_state)
                &&
                (CIS_STATE_CONNECTED ==
                    remote_ascs[ascs_index].ascs.ase[ase_index_using_same_cis].cig_cis_info.cis_state))
            {
      ...
      )
{
    ...
    APPL_DBG("[APPL][GA][UCC]: >> appl_ga_ucc_get_cig_cis_shared_bidirectional_ase_index\n");
     ...
     )
{
    ...
    for (i = 0U; i < cis_count; i++)
    {
        ...
        if ((MAX_REM_ASCS_INFO == ascs_index) ||
            (INVALID_CONN_HANDLE == cis_handle))
        {
            APPL_ERR
            (
          ...
          )
{
    ...
    ascs_index = 0U;
    ase_index = 0U;
    acl_handle = INVALID_CONN_HANDLE;
    cis_handle_index = 0U;
    ...
    for (ascs_index = 0U; ascs_index < MAX_REM_ASCS_INFO; ascs_index++)
    {
        /* Check for valid connection device index */
        if (INVALID_CONN_DEV_INDEX != remote_ascs[ascs_index].conn_dev_index)
        {
            /* Get the ACL handle */
            retval = appl_ga_utils_get_conn_handle_from_index
                     (
        ...
        if (GA_ROLE_SOURCE == local_role)
        {
                              ...
                              ))
            {
                retval = appl_ga_utils_audio_src_le_setup_iso_data_path
                         (
                             remote_ascs[ascs_index].ascs.ase[ase_index].cig_cis_info.handle,
                             &remote_ascs[ascs_index].ascs.ase[ase_index].stream_symbol
                         );
