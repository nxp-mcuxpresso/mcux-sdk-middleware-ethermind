...
#if ((defined APPL_MICP_CONTROLLER) || (defined APPL_MICP_DEVICE))
...
void appl_micp_ct_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 7:
            ...
            if (1U == (UINT8)choice)
            {
                CONSOLE_OUT("Enter the MICS Handle: ");
                CONSOLE_IN("%d", &choice);
                if (MICP_MAX_MICS_CONTEXTS <= (UINT8)choice)
                {
                    APPL_INF("Invalid handle !\n");
                }
                else
                {
                    ga_micp_context = (UINT8)choice;
                }
            }
        ...
        case GA_MC_AICS_SET_AUDIO_INPUT_DESC_IND:
        {
            ...
            if (micp_srvc_inst < MICP_MAX_AICS_ENTITIES)
            {
                           ...
                           )))
                {
                     ...
                     */
                    aics_char_val_change_flag = GA_TRUE;
                }
            }
            else
            {
          ...
          )
{
    ...
    if (mcd_event_info->srv_inst >= MICP_MAX_AICS_CONTEXTS )
    {
...
void appl_ga_micp_client_init()
{
    ...
    APPL_TRC("APPL_MICP_CONTROLLER not defined!");
#endif /* APPL_MICP_CONTROLLER */
}

void appl_ga_micp_client_shutdown()
{
    GA_RESULT retval;
...
void appl_ga_micp_server_init()
{
    ...
    APPL_TRC("APPL_MICP_DEVICE not defined!");
#endif /* APPL_MICP_DEVICE */
}

void appl_ga_micp_server_shutdown()
{
    GA_RESULT retval;
...
#endif /*  ((defined APPL_MICP_CONTROLLER) || (defined APPL_MICP_DEVICE))  */
