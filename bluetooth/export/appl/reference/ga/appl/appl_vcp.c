...
int appl_vcp_ct_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 9:
            ...
            if (1U == (UINT8)choice)
            {
                CONSOLE_OUT("Enter VCS Handle :");
                CONSOLE_IN("%d", &choice);

                if (VCP_MAX_VCS_CONTEXTS <= (UINT8)choice)
                {
                    APPL_INF("Invalid Handle !\n");
                }
                else
                {
                    appl_vcp_ct_hndl = (UCHAR)choice;
                }
            }
        ...
        case 36:
        {
            ...
            appl_audio_input[1U] = (INT8)input;
            appl_audio_input[0U] =
...
int appl_vcp_rd_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 61:
        {
            ...
            aics_input_gain_setting[0U].gain_settting_min = (INT8 )input;

            CONSOLE_OUT("Enter AICS Gain Setting Max. (<= 127):");
            CONSOLE_IN("%d", &input);

            aics_input_gain_setting[0U].gain_setting_max = (INT8)input;
        }
            break;
    ...
    case GA_VC_AICS_SET_AUDIO_INPUT_DESC_IND:
    {
        ...
        if (vcp_srvc_inst < VCP_MAX_AICS_ENTITIES)
        {
                       ...
                       )))
            {
                ...
                /* Reset Audio Input Description before updating */
                GA_mem_set
                (
                    &aics_chr_info[vcp_srvc_inst].audio_in_desc[0U],
                    0x00U,
          ...
          )
{
    ...
    {
        APPL_ERR("Invalid VCS Service Instance, 0x%02X !\n", vcp_event_info->srvc_inst);
        return GA_FAILURE;
    }
    ...
    if (2U > datalen)
    {
        APPL_ERR("\nInvalid CP Write Length, 0x%02X !\n", datalen);
        return GA_FAILURE;
    }
    ...
    {
        APPL_ERR("\nInvalid ControlPoint Opcode, 0x%02X !\n", cp_op_code);
          ...
          )
{
    ...
    if (AICS_CP_OP_SET_GAIN_SETTING == cp_op_code)
    {
        ...
        if (((INT8)data[1U] < aics_chr_info[srvc_inst].gain_setting->gain_settting_min) ||
            ((INT8)data[1U] > aics_chr_info[srvc_inst].gain_setting->gain_setting_max))
        {
            return AICS_ERR_CODE_VALUE_OUT_OF_RANGE;
        }
        else if (aics_chr_info[srvc_inst].input_state->gain_setting != (INT8)data[1U])
        {
            aics_chr_info[srvc_inst].input_state->gain_setting = (INT8)data[1U];
...
void appl_ga_vcp_client_init(void)
{
    GA_RESULT retval;
#ifdef APPL_VCP_CONTROLLER
    ...
    APPL_TRC("APPL_VCP_CONTROLLER not defined!");
#endif /* APPL_VCP_CONTROLLER */
}

void appl_ga_vcp_client_shutdown(void)
{
    GA_RESULT retval;
#ifdef APPL_VCP_CONTROLLER
...
void appl_ga_vcp_server_init(void)
{
    GA_RESULT retval;
    ...
    APPL_TRC("APPL_VCP_RENDERER not defined!");
#endif /* APPL_VCP_RENDERER */
}

void appl_ga_vcp_server_shutdown(void)
{
    GA_RESULT retval;
...
#endif /* ((defined APPL_VCP_CONTROLLER) && (defined APPL_VCP_RENDERER)) */
