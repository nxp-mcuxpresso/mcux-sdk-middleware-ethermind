...
/* Maximum TBS Instances supported by CCP */
#define TBS_MAX_INST 5U

/* Unused TBS Instance */
#define TBS_INST_FREE 0xFU
...
DECL_STATIC CCP_HANDLE appl_ccp_gtbs_handle;
DECL_STATIC CCP_HANDLE appl_ccp_tbs_handle[TBS_MAX_INST];
DECL_STATIC CCP_HANDLE appl_ccp_handle_in_use;
...
/* --------------------------------------------- Functions */
GA_RESULT appl_ccp_ce_read_sess_inst_for_op(void );
void appl_find_empty_tbs_pos(UINT8 * index);
...
void main_ccp_ce_menu_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 6:
        {
            ...
            {
                APPL_INF("Maximum TBS Instances reached\n");
            }
        }
        ...
        case 8:
        {
            ...
            if (1 == choice)
            {
                ...
                retval = GA_ccp_ce_release_tbs(appl_ccp_handle_in_use);
                APPL_TRC("Retval - ");
                appl_ccp_display_ga_result(retval);
            }
            else if (2 == choice)
            {
                ...
                retval = GA_ccp_ce_close_tbs(appl_ccp_handle_in_use);
                APPL_TRC("Retval - ");
                appl_ccp_display_ga_result(retval);
            }
            else
        ...
        case 9:
            appl_ccp_ce_read_sess_inst_for_op();
            break;
...
#ifdef CCP_SUPPORT_CONTEXT_MANAGE
        {
            for (i = 0U; i < CCP_MAX_TBS_CONTEXTS; i++)
            {
                ...
                else
                {
                    ...
                    if (GA_SUCCESS == retval)
                    {
                        appl_ccp_tbs_handle[i] = appl_ccp_tbs_manage_ctx_info[i].appl_ccp_tbs_handle_access;
...
GA_RESULT appl_ccp_ce_read_sess_inst_for_op(void )
{
    ...
    /* Reset */
    appl_ccp_handle_in_use = 0U;

    CONSOLE_OUT("Session Instance Type: [TBS:0] [GTBS: 1]\n");
    CONSOLE_IN("%d", &input);
    if (0x01U == (UINT8)input)
    {
        appl_ccp_handle_in_use |= CCP_CE_GTBS_ENABLE;
    }

    CONSOLE_OUT("Session Instance ID: \n");
    CONSOLE_IN("%d", &input);
    appl_ccp_handle_in_use |= (UINT8)input;
...
void appl_find_empty_tbs_pos(UINT8 * index)
{
    ...
    for (i = 0U; i < TBS_MAX_INST; i++)
    {
        if (TBS_INST_FREE == appl_ccp_tbs_handle[i])
        {
            *index = i;
            break;
        }
