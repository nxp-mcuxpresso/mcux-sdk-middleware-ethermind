...
================ Service Management ~ Optional Service ============ \n\
    ...
    8. Remove MCS Instance: VLC/MPC Player \n\
\n\
...
{
    /* Object Type - Track */
    GA_OTS_OBJ_TYPE_TRACK_SEGMENTS,
...
void appl_mcp_se_send_ntf_curr_track_pos_timer_expiry_handler
          (
              void   * data,
              UINT16   datalen
          );
void appl_mcp_se_send_ntf_curr_playback_speed_timer_expiry_handler
          (
              void   * data,
              UINT16   datalen
          );
#ifdef MCP_SUPPORT_OBJECT_TRANSFER
void appl_mcp_se_send_ntf_curr_track_obj_id_timer_expiry_handler
          (
              void   * data,
              UINT16   datalen
          );
...
#endif /* MCP_SUPPORT_OBJECT_TRANSFER */
void appl_mcp_se_send_ntf_playing_order_timer_expiry_handler
          (
              void   * data,
              UINT16   datalen
          );
#ifdef MCP_SUPPORT_OBJECT_TRANSFER
void appl_mcp_se_send_ntf_search_res_obj_id_timer_expiry_handler
          (
              void   * data,
              UINT16   datalen
          );
...
void main_mcp_se_menu_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 8:
            CONSOLE_OUT("Enter Media Player to be rempved: [1: VLC] [2:MPC] ");
            CONSOLE_IN("%d", &input);
            APPL_TRC("GA_mcp_se_unregister_mcs...\n");
            if (APPL_MCP_VLC_PLAYER == input)
            {
                retval = GA_mcp_se_unregister_mcs(appl_mcp_se_vlc_handle);
            }
            else if (APPL_MCP_MPC_PLAYER == input)
            {
                retval = GA_mcp_se_unregister_mcs(appl_mcp_se_mpc_handle);
            }
            else
            {
                break;
          ...
          )
{
    ...
    case MCP_SE_OTP_SE_READ_OBJ_NAME_IND:
        ...
        if (NULL != appl_otp_se_curr_obj)
        {
            rsp = (CHAR *)&appl_otp_se_curr_obj->obj_name[0U];
        ...
        {
            /* Object is not Set, Read is received. TODO: Send Error? */
        }
        break;
     ...
     )
{
    ...
    opcode = ots_cntrl_point_rsp_param->char_info[1U];
    GA_COPY_BRR_DEVICE(&device, &ots_cntrl_point_rsp_param->dev);
    ...
    if (MCP_OTS_OACP_READ_OPCODE == opcode)
    {
...
void appl_mcp_se_otp_se_parse_oacp_req(GA_ENDPOINT * device, MCP_SE_EVT * evt)
{
    ...
    (
        ...
        &ots_oacp_cntrl_rsp,
        sizeof(APPL_CNTRL_POINT_RSP_PARAM)
    );
}
           ...
           )
{
    ...
    else if (offset > MCP_OBJ_ID_LEN)
    {
        SET_CNTRL_PNT_RSP_RESULT_CODE(MCP_OTS_OACP_INVALID_PARAMETER);
    }
    else if ((offset + length) > MCP_OBJ_ID_LEN)
    {
        SET_CNTRL_PNT_RSP_RESULT_CODE(MCP_OTS_OACP_INVALID_PARAMETER);
    }
    else if (0U)
...
void appl_mcp_se_register_mcs(void)
{
    int input;
    GA_RESULT retval;
    MCP_SE_HANDLE appl_mcp_se_handle;
    ...
    if (GA_SUCCESS == retval)
    {
        if (APPL_MCP_VLC_PLAYER == input)
        {
            ...
            /*
             * To handle crash when OLCP done without current object being set.
             * TODO: Check the protocol expected
             */
            appl_otp_se_curr_obj = &appl_mcp_se_icon_obj_vlc;
        }
        else if (APPL_MCP_MPC_PLAYER == input)
