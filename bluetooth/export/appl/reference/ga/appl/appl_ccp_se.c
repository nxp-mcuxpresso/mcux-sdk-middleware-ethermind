...
void main_ccp_se_menu_operations(void)
{
    ...
    GA_LOOP_FOREVER()
    {
        ...
        case 5:
            APPL_TRC("GA_ccp_se_unregister_tbs...\n");
            retval = GA_ccp_se_unregister_tbs(tbs_sess_inst_id);
        ...
        case 9:
            ...
            CONSOLE_IN("%d", &input);
            if (APPL_TBS_SE_ENITITES < (UINT8)input)
            {
                APPL_INF("Invalid handle !\n");
            }
            else
            {
                tbs_sess_inst_id = (UINT8)input;
            }
            break;
        ...
        case 10:
            ...
            appl_ccp_se_init_gtbs_info(&appl_tbs_se_params[i]);
            i++;

            if (APPL_TBS_SE_ENITITES > i)
            {
                appl_ccp_se_init_tbs_info(&appl_tbs_se_params[i], i);
            }
            i++;
        ...
        case 60:
        {
            ...
            retval = appl_tbs_setup_incall(tbs_sess_inst_id, &index);
            APPL_TRC("retval: 0x%04x\n", retval);
            APPL_TRC("call index in call list:0x%02x \n", index);

            if (GA_SUCCESS == retval)
            {
                appl_tbs_notify_incall_details(tbs_sess_inst_id, index);
            }
        ...
        case 73:
            ...
            if (GA_SUCCESS == retval)
            {
                appl_gtbs_set_remotely_held(tbs_sess_inst_id, call_index);
            }
            else
            {
        ...
        case 77:
            ...
            if (GA_SUCCESS == retval)
            {
                appl_gtbs_set_active(tbs_sess_inst_id, call_index);
            }
            else
            {
                APPL_ERR("Invalid Call ID !\n");
        ...
        case 78:
            ...
            if (GA_SUCCESS == retval)
            {
                appl_gtbs_set_call_alerting(tbs_sess_inst_id, call_index);
            }
            else
            {
                APPL_ERR("Invalid Call ID !\n");
          ...
          )
{
    ...
    case CCP_SE_RD_IND:
        ...
        if ((APPL_TBS_SE_ENITITES <= appl_event_info->srvc_handle) ||
           ...
           )
{
    ...
    case TBS_CP_OP_JOIN:
        ...
        default:
             ...
             */
            /* Copy service handle */
            ccp_se_ntf_info.tbs_handle = appl_event_info->srvc_handle;
            /* Copy Char UUID*/
            ccp_se_ntf_info.char_uuid = GA_CHAR_TBS_CALL_CP;
            /* Copy service instance */
            ccp_se_ntf_info.srvc_inst = appl_event_info->srvc_handle;
            /* Copy device info */
...
void appl_tbs_notify_incall_details(UCHAR tbs_srvc_index, UCHAR index)
{
    ...
    retval = appl_ccp_se_pack_tbs_chr_value
             (
                ...
                &appl_tbs_se_params[tbs_srvc_index],
                &appl_chr_data[0U],
                &appl_chr_datalen
             );
...
void appl_tbs_set_call_state(void)
{
...
#endif /* 0 */
    {
        ...
        /*
         * If the Call_ID doesn't exist, then its a new call. Get a free index
         * to update
         */
        if (GA_FAILURE == retval)
        {
            ...
            APPL_TRC
            (
                "Call_ID not present. This is a new Call ID: 0x%02X\n",
                appl_tbs_se_params[srvc_index].call_state[call_index].call_index
            );
        }
...
void appl_tbs_set_call_termination_reason(void)
{
    int        input;
    UCHAR      srvc_index;
    UINT8      index;
    ...
    if (GA_SUCCESS == retval)
    {
        ...
        {
            APPL_ERR("Invalid Call State !\n");
        }
        appl_tbs_se_params[srvc_index].term_reason->call_indx = (UCHAR)input;

        CONSOLE_OUT("Enter Reason\n");
        CONSOLE_IN("%d", &input);
        appl_tbs_se_params[srvc_index].term_reason->reason = (UCHAR)input;
    }
    else
    {
        APPL_ERR("Invalid Call Index !\n");
...
void appl_tbs_set_char_value(void)
{
    ...
    CONSOLE_OUT("15 -> Set Call Termination Reason \n");
    CONSOLE_IN("%d", &rd_val);
    ...
    case 15:
        appl_tbs_set_call_termination_reason();
        break;
...
void appl_tbs_notify_bearer_provider_name(void)
{
    ...
    retval = appl_ccp_se_pack_tbs_chr_value
             (
                 ...
                 &appl_tbs_se_params[tbs_sess_inst_id],
                 &appl_chr_data[0U],
                 &appl_chr_datalen
             );
...
void appl_ccp_se_call_state_timer_handler(void * data, UINT16 datalen)
{
    ...
    else
    {
        ...
        if (APPL_TBS_SE_ENITITES <= ccp_se_ntf_info->srvc_inst)
        {
            /* Should not reach here */
...
void appl_ccp_se_register_tbs(void)
{
    GA_RESULT retval;
