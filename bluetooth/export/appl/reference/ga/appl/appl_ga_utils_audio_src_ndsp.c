          ...
          )
{
    ...
    if (AUDIO_LC3_IN_PROGRESS != audio_lc3_src.state)
    {
		...
		lc3_retval = lc3_ndsp_enc_set_config_params
					(
						&audio_lc3_src.ctx.config,
						sf_hz,					//the sampling rate
						cc,						//number of channels.  If CIS/BIS, = 1
						fd_as_per_lc3_format,	//frame period
						bps						//the compressed data rate
					);

		if (lc3_retval == LC3_ENCODER_SUCCESS)
		{
			for (uint8_t chan_id = 0; (!lc3_retval) && (chan_id < cc); chan_id++)
			{
				//inititialize the ch cnxt
				lc3_retval = lc3_enc_init_ch_cntx
							(
								&audio_lc3_src.ctx.enc_ctx[chan_id],
								&audio_lc3_src.ctx.config,
								audio_lc3_src.ctx.target_enc_bytes[chan_id]
							);
			}
		}
        ...
        else
        {
            ...
            (
                ...
                0U,
                sizeof(audio_lc3_src.ctx.enc_ctx)
            );
            retval = GA_FAILURE;
        }
    }
          ...
          )
{
    ...
    UINT8     cc[3U] = { 0 }; /* TODO */
    UINT8     stream_symbol[3U] = { 0 }; /* TODO */
    UINT16    * seq_num[3U] = { 0 }; /* TODO */
    UINT16      ts_flag[3U] = { 0 }; /* TODO */
    ...
    for (index = 0U; index < AUDIO_SRC_ISO_MAX_ENTRIES; index++)
    {
        if (GA_TRUE == audio_iso_src[index].streaming_state)
        {
            ...
            stream_symbol[active_handle_count] = audio_iso_src[index].stream_symbol;
    ...
    (
        ...
        (pcm_datalen >> 1U), /* This is 1CC Source Buffer size, Conversion from 16bits to 8bits hence (/2)(>> 1U)*/
        audio_lc3_src.cc
    );
    ...
    if (AUDIO_LC3_IN_PROGRESS == audio_lc3_src.state)
    {
    	for (uint8_t chan_in = 0; chan_in < audio_lc3_src.cc; chan_in++)
    	{
        	lc3_encoded_bytes = LC3_encode_a_frame
        						(
        							&audio_lc3_src.ctx.enc_ctx[chan_in],
        							&audio_lc3_src.ctx.src_pcm_buffer[chan_in][0],
    								&audio_lc3_src.ctx.src_enc_buffer[chan_in][0],
			...
			if (lc3_encoded_bytes > 0)
			{
				for (index = 0U; index < active_handle_count; index++)
				{
					...
					(
						...
						lc3_encoded_bytes, /* This is 1CC Encoded Byte size */
						cc[index]
					);
					(*seq_num[index])++;
					...
					retval = audio_src_iso_write
							 (
								 ...
								 *(seq_num[index]),
								 &enc_interleaved_data[0U],
								 ((UINT16)lc3_encoded_bytes * cc[index])
							 );
...
{
    return lc3_encoder_convert_bitrate_to_bytes(bitrate_in_bps,sf_in_Hz,fd_as_per_lc3_format);
}
