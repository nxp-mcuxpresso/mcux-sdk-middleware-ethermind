...
#ifdef AVRCP_COVER_ART_RESPONDER
    printf ("Initializing AVRCP Cover Art Responder...\n");
...
#ifdef AVRCP_COVER_ART_RESPONDER
    printf ("Shutting down AVRCP  Cover Art Responder...\n");
...
void appl_avrcp_tg_start (UCHAR index)
{
    ...
    printf ("Starting AVRCP Cover Art Responder Instance...\n");

    /* this might not return success value for 2nd instance,
     * currently max value of avrcp CAR value is set to 1 in BT_limits.h
...
void appl_avrcp_tg_stop (UCHAR index)
{
    ...
    {
        printf ("Stopping AVRCP Cover Art Responder Instance...\n");
     ...
     )
{
    ...
    case AVRCP_OPCODE_UNIT_INFO:
        ...
        if (API_SUCCESS != retval)
        {
            printf("Failed to send AVRCP_OPCODE_UNIT_INFO: Error Code = 0x%04X\n", retval);
        }
    ...
    case AVRCP_OPCODE_SUBUNIT_INFO:
        ...
        if (API_SUCCESS != retval)
        {
            printf("Failed to send AVRCP_OPCODE_SUBUNIT_INFO: Error Code = 0x%04X\n", retval);
        }
...
#ifdef MULTI_PROFILE_FSM_SUPPORT
        if (AVRCP_OPERATION_ID_PLAY == op_id)
        {
            /* call start stream on release */
            if (op_id_state != 0x00)
            {
                printf("play for inst = %d\n", inst);
                appl_a2dp_stream_start(inst);
            }
        }
        else if (AVRCP_OPERATION_ID_PAUSE == op_id)
        {
            /* call suspend stream on release */
            if (op_id_state != 0x00)
            {
                printf("pause for inst = %d\n", inst);
                appl_a2dp_stream_suspend(inst);
            }
        }
#endif /* MULTI_PROFILE_FSM_SUPPORT */
        printf("    State = ");
...
#ifdef AVRCP_BRIDGING
UCHAR appl_get_avrcp_tl_message_type()
{
    return temp_tl_type;
}
#endif /* AVRCP_BRIDGING */
...
#ifndef AVRCP_BRIDGING /* non bridging */
    ...
    case AVRCP_METADATA_PDU_ID_GET_CAPABILITY:
        {
            ...
            printf ("    PDU-ID -> Get Capability <0x%x>\n", vd_cmd_info->pdu_id);
...
SEND_METADATA_PDU_RSP:
        if (API_SUCCESS == retval)
        {
            ...
            {
                printf ("    Sending Reject Response\n");
     ...
     )
{
    ...
    else
    {
        ...
        {
            printf ("No. of Attributes > APPL_AVRCP_MAX_ATTR_COUNT\n");
     ...
     )
{
    ...
    else
    {
        ...
        printf ("    PDU-ID -> Get Play Status<0x%x>.\n", vd_cmd_info->pdu_id);
...
#ifdef MULTI_PROFILE_FSM_SUPPORT
        a2dp_audio_set_speaker_volume_pl(system_volume);
#endif /* MULTI_PROFILE_FSM_SUPPORT */
     ...
     )
{
    ...
    else
    {
        ...
        vd_cmd_data = vd_cmd_info->vd_cmd_data;
#ifdef MULTI_PROFILE_FSM_SUPPORT
        vd_cmd_datalen = vd_cmd_info->vd_cmd_datalen;
#endif /* MULTI_PROFILE_FSM_SUPPORT */
        param_offset = 11U;
...
#ifdef MULTI_PROFILE_FSM_SUPPORT
        media_name_len = vd_cmd_datalen - param_offset;
        printf ("     - media_name_len : %d\n", media_name_len);
        BT_mem_copy(media_name, &vd_cmd_data[param_offset], media_name_len);
        media_name[media_name_len] = '\0';
        param_offset += media_name_len;
#endif
        ...
        /* Display */
        printf ("     - Scope : %d\n", scope);
        printf ("     - UID   : 0x0%04x%04x\n", uid[0U], uid[1U]);
        printf ("     - UID counter : 0x%04x\n", uid_counter);
#ifdef MULTI_PROFILE_FSM_SUPPORT
#ifdef A2DP_PL_SOURCE_FS_MEDIA
        printf ("     - Media Name  : %s\n", media_name);
        audio_set_media_file_name_pl(media_name);
...
SEND_RSP:
        ...
        if (0x00U != reject)
        {
            ...
            if (API_SUCCESS != retval)
            {
                printf("BT_avrcp_al_send_browsing_cmd_rsp failed - 0x%04X\n", retval);
            }
        }
    }
     ...
     )
{
    ...
    if (NULL == data)
    {
        printf("Failed to Allocated memory for media_player_list\n");
        /* return; */
    }
    else
    {
     ...
     )
{
    ...
    else
    {
       ...
       /* offset till parameter length */
        ...
        if (0x00U == direction)
        {
            printf ("    Direction : Up(0x%0x)\n", direction);
        }
        else
        {
            printf ("    Direction : Down(0x%0x)\n", direction);
        }
     ...
     )
{
    ...
    else
    {
       ...
       /* offset till parameter length */
        ...
        printf ("    PDU ID - Get Item Attributes (0x%x).\n", brow_cmd_info->pdu_id);
        ...
        printf ("     - UID   : 0x0%04x%04x\n", uid[0U], uid[1U]);
        printf ("     - UID counter : 0x%04x\n", uid_counter);
        printf ("     - No. of Attributes: ");

        if (uid_counter != global_uid_counter)
        {
            printf (" UID counter does not match\n");
            reject = 0x01U;
        ...
        if (0x00U == num_attrs)
        {
            printf ("All Supported Attributes\n");
        }
        else if (0xFFU == num_attrs)
        {
            printf ("Zero Atrributes\n");
     ...
     )
{
    ...
    printf ("Select Event ID to be Generated\n");
    ...
    printf ("    e. Reject register notifications\n");
    ...
    if (0xFFU == choice)
    {
        ...
        rsp_pdu_info.vd_cmd_data    = &event_id;
        rsp_pdu_info.vd_cmd_datalen = 1U;
    }
            ...
            )
{
    ...
    {
        printf ("AVRCP_TG Cover Art Handle value out of range\n");
    ...
    else
    {
        ...
        case AVRCP_CAR_TRANSPORT_CLOSE_CNF:
            ...
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);

            break;

        case AVRCP_CAR_CONNECT_IND:
            printf ("Recvd AVRCP_CAR_CONNECT_IND - %04X\n", event_result);
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);
        ...
        case AVRCP_CAR_DISCONNECT_IND:
            printf ("Recvd AVRCP_CAR_DISCONNECT_IND - 0x%04X\n", event_result);
            printf ("AVRCP-CT Cover Art Handle - 0x%02X\n", handle);

            break;

        case AVRCP_CAR_GET_IMAGE_PROPERTIES_IND:
            printf ("Recvd AVRCP_CAR_GET_IMAGE_PROPERTIES_IND - %04X\n",
                event_result);
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);
        ...
        case AVRCP_CAR_GET_IMAGE_IND:
            ...
            printf ("Recvd AVRCP_CAR_GET_IMAGE_IND - %04X\n",
                event_result);
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);
            ...
            if (sent == 0U)
            {
                ...
                if (NULL == image_fp)
                {
                    ...
                    else
                    {
                        ...
                        printf("Sending OBEX_SERVER_ERR_RSP response\n");
        ...
        case AVRCP_CAR_GET_LINKED_THUMBNAIL_IND:
            printf ("Recvd AVRCP_CAR_GET_LINKED_THUMBNAIL_IND - %04X\n",
                event_result);
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);
        ...
        case AVRCP_CAR_GET_ABORT_IND:
            ...
            printf ("Recvd AVRCP_CAR_GET_ABORT_IND - %04X\n",
                     event_result);
            printf ("AVRCP-TG Cover Art Handle - 0x%02X\n", handle);
        ...
        if (0U != send_response)
        {
            ...
            if (API_SUCCESS != retval)
            {
                printf ("Failed to send AVRCP_TG Cover Art Response - 0x%05X\n", retval);
            }
...
API_RESULT appl_handle_avrcp_bridging_cmd_ind(uint8_t inst)
{
	    ...
	    avrcp_cmd_info = appl_avrcp_get_cmd_info();
	    vd_cmd_info = appl_avrcp_get_vendor_cmd_info();
	    if(avrcp_cmd_info != NULL)
	    {
	    	/* Update Transaction Label and Message Type */
			avctp_tl_msg_type = avrcp_cmd_info->cmd_tl | AVCTP_CTR_MSG_RESPONSE;

			if((avrcp_cmd_info->cmd_opcode) == AVRCP_OPCODE_PASS_THROUGH)
			{
				AVRCP_GET_MSG_OPERATION_INFO((UCHAR *)avrcp_cmd_info->cmd_info, op_id, op_id_state);
				...
				temp_tl_type = avctp_tl_msg_type;
				retval = BT_avrcp_al_send_passthrough_cmd_rsp
						(
	    ...
	    /* handle vendor command from here */
	    if (vd_cmd_info != NULL)
        {
			param_offset    = 0U;
			cmd_info        = (UCHAR *)vd_cmd_info->vd_cmd_data;
			...
			switch(vd_cmd_info->pdu_id)
			{
				case AVRCP_METADATA_PDU_ID_REGISTER_NOTIFICATION:
					printf ("    PDU-ID -> Register Notification<0x%x>.\n", vd_cmd_info->pdu_id);
					if((inst != PROFILE_INSTANCE_INVALID) && (sec_avrcp_index != PROFILE_INSTANCE_INVALID))
					{
