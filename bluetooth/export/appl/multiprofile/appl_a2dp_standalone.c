...
#include "appl_a2dp_standalone.h"
#include "a2dp_pl.h"
...
/* NOTE: A2DP_BRIDGING_APP can be enabled from Project settings */
#ifdef A2DP_BRIDGING_APP
#undef APPL_A2DP_DEFAULT_CONFIG
#ifdef A2DP_SUPPORT_MULTIPLE_MEDIA_FRAME_WRITE
#error Disable A2DP_SUPPORT_MULTIPLE_MEDIA_FRAME_WRITE when A2DP_BRIDGING_APP is defined
#endif
/* TODO: add #error directive to throw compilation error if AVDTP_HAVE_CONTENT_PROTECTION is enabled */
#define APPL_A2DP_AVOID_FILE_DEFAULT_CONFIG    -1U
#endif 
 ...
 * This value shall be at least double of the data length being equeued every time (app_standalone_a2dp_src_enqueue).
 */
...
void appl_standalone_a2dp_src_prepare(void);

API_RESULT app_standalone_a2dp_src_enqueue
           (
               /* IN */  const UCHAR   * data,
               /* IN */  UINT16          datalen
           );
...
API_RESULT a2dp_standalone_encode_n_send
           (
               UINT8    a2dp_instance,
               UCHAR  * pcm_data,
               UINT16   pcm_datalen
...
void main_a2dp_operations (void)
{
    ...
    BT_LOOP_FOREVER()
    {
        ...
        case 2:
        	printf("Initializing A2DP ... "); fflush (stdout);
            /*printf("appl_debug_enabled is %d\n", appl_debug_enabled);*/
            retval = BT_a2dp_init();
...
#ifdef A2DP_BRIDGING_APP
                    appl_a2dp_connect_info_reset((UCHAR)choice);
#endif
                }
...
#endif /* JPL_USE_APP_MEMORY */
#endif /* A2DP_BRIDGING_APP */
#endif /* A2DP_SINK */
            appl_a2dp_state = APPL_A2DP_INITIALSED;
#ifdef A2DP_BRIDGING_APP
            printf("\n THIS IS AN APPLICATION FOR A2DP BRIDGE \n");
#endif /* A2DP_BRIDGING_APP */
            break;

        case 3:
        	printf("Shutdown A2DP ... "); fflush (stdout);
            ...
            {
            	printf("Invalid A2dp state 0x%02X\n",appl_a2dp_state);
            }
...
#else
            printf ("Please Enable AVDTP_HAVE_GET_ALL_CAPABILITIES Flag\n");
...
#else
            printf ("Please Enable A2DP_SINK Flag\n");
#endif /* A2DP_SINK */
            break;
        ...
        case 16:
            ...
            break;
#else
            printf ("Please Enable A2DP_1_3 Flag \n");
#endif  /* A2DP_1_3 */
        case 17:
        	printf("\n");
        ...
        case 18:
            ...
            {
                printf ("ACL Connection for A2DP not found\n");
...
#else /* AVDTP_ASSISTIVE_MODE */
            printf("Assistive A2DP feature is not supported for the platform.\n");
#endif /* AVDTP_ASSISTIVE_MODE */
            break;
...
#else
            printf ("Please Enable AVDTP_HAVE_CONTENT_PROTECTION \
...
#else
             printf("HAVE_AVDTP_TEST_MODE feature flag not defined\n");
...
void appl_a2dp_register_codec (void)
{
    ...
    else
    {
        ...
        printf("\n");
        printf("Select Streaming End Point Type.  Source(0)/Sink(1)... "); fflush(stdout);
        ...
        printf("\n");
        printf ("Select Codec Type(in Hex):\n");
        printf ("    0->SBC\n");
#ifdef A2DP_MPEG_1_2
        printf ("    1->MPEG-1,2(MP3)\n");
#endif /* A2DP_MPEG_1_2 */
#ifdef A2DP_MPEG_2_4
        printf ("    2->MPEG-2,4(AAC)\n");
#endif /* A2DP_MPEG_2_4 */
#ifdef A2DP_VENDOR_CODEC
        printf ("   FF->Vendor Specific\n");
#endif /* A2DP_VENDOR_CODEC */
        ...
        else if (AVDTP_CODEC_AUDIO_VENDOR_SPECIFIC == codec_type)
        {
            ...
            printf ("Enter Vendor Codec ID(in Hex): ");
            scanf ("%x", &val);
            ...
            printf ("Enter Vendor Specific Codec Value Length: ");
            scanf ("%d", &val);

            if ((6U + val) > AVDTP_MAX_CODEC_IE_LEN)
            {
                printf ("\n== AVDTP_MAX_CODEC_IE_LEN is NOT sufficient for the Codec Info.== \n");
                printf ("== Update AVDTP_MAX_CODEC_IE_LEN definition as required ==\n");
            ...
            else
            {
                ...
                printf ("Enter Vendor Specific Codec Value(in Hex): ");
        ...
        {
            printf ("Unsupported Codec Type 0x%02X. Registration Failed\n",
            codec_type);
        ...
        if (API_SUCCESS == retval)
        {
            ...
            /* Register to A2DP */
            printf("A2DP Register Codec ... "); fflush (stdout);
            ...
            else
            {
                ...
                {
                    printf ("Set A2DP Sink Media MTU ...\n");
...
void appl_a2dp_connect (void)
{
    ...
    if (API_SUCCESS == retval)
    {
        ...
        if (ret < 0)
        {
            printf ("Failed to read Codec IE Configuration\n");
        }
        else
        {
#ifdef A2DP_VENDOR_CODEC
            if (AVDTP_CODEC_AUDIO_VENDOR_SPECIFIC == appl_a2dp_instance[indx].sep_cap.codec_cap.codec_type)
            {
                ...
                if (API_SUCCESS == retval)
                {
                    ...
                    {
                        printf ("Codec IE Config. length and Codec IE Cap. length does NOT match\n");
                        printf ("Returing....\n");
                        retval = API_FAILURE; /* return; */
                    }
                }
            ...
            if (API_SUCCESS == retval)
            {
                ...
                /* Get Remote SEID */
                printf("Enter Remote SEID (0x) = "); fflush (stdout);
                scanf("%x", &choice);
                remote_seid = (UCHAR) choice;
                if (remote_seid == 0)
                {
                    printf("Remote SEID is 0x00\n");
                    printf("=> A2DP will perform AVDTP Discover & Get Capabilities\n");
                }

                /* A2DP Connect */
                printf("A2DP Connect ... "); fflush (stdout);
...
void appl_a2dp_reconfigure (UCHAR indx)
{
    ...
    if (indx >= A2DP_MAX_CODECS)
    {
        printf("Instance is outof Range\n");
        /* return; */
    }
    else
    {
        ...
        else
        {
            ...
            if (ret >= 0)
            {
                ...
                /* A2DP Reconfigure */
                printf("A2DP Reconfigure ... "); fflush (stdout);
...
void appl_a2dp_disconnect (void)
{
    ...
    else
    {
        ...
        {
            /* A2DP Disconnect */
            printf("A2DP Disconnect ... "); fflush (stdout);
            ...
            {
                printf("OK. Wait for A2DP_DISCONNECT_CNF\n");
            }
        }
    }
...
void appl_a2dp_set_media_mtu (void)
{
    ...
    else
    {
        ...
        {
            printf("Enter Media MTU = "); fflush (stdout);
            scanf("%d", &choice);
            if ((choice < 48U) || (choice > L2CAP_MAX_MTU))
            {
                printf("*** Invalid Media MTU - %d\n", choice);
                /* return; */
            }
            else
            {
                ...
                /* A2DP Set Media MTU */
                printf("A2DP Set Media MTU ... "); fflush (stdout);
...
void appl_a2dp_set_cp_header (void)
{
    ...
    else
    {
        ...
        {
            printf ("1. COPY RESTRICTED\n2. COPY ONCE ONLY\n3. COPY UNLIMITED\n");
            printf("Enter SCMS-T CP Type: "); fflush (stdout);
            scanf("%d", &choice);
           ...
           )
{
    ...
    case A2DP_CONNECT_CNF:
        ...
        if (API_SUCCESS == event_result)
        {
            ...
            appl_a2dp_instance[codec_instance].a2dp_connection_state = APPL_A2DP_CONNECTED;
#endif //A2DP_BRIDGING_APP
        }
#ifdef AVDTP_HAVE_CONTENT_PROTECTION
        else
    ...
    case A2DP_CONNECT_IND:
        ...
        if (API_SUCCESS == event_result)
        {
            ...
            for (i = 0U; i < A2DP_MAX_CODECS; i++)
            {
                ...
                */
                if ((i != codec_instance) &&
                    (appl_a2dp_instance[i].role != appl_a2dp_instance[codec_instance].role)&&
					(appl_a2dp_instance[i].a2dp_connection_state == APPL_A2DP_CONNECTED) &&
                    (memcmp(appl_a2dp_instance[i].codec_ie_conf, appl_a2dp_instance[codec_instance].codec_ie_conf, AVDTP_MAX_CODEC_IE_LEN) != 0))
                {
                    /* Get SEP Configuration Codec IE */
                    codec_ie_conf = appl_a2dp_instance[i].codec_ie_conf;

                    /* Enable Codec Capabilities in SEP Configuration */
                    AVDTP_SET_SEP_CONFIGURATION_CODEC_CAP
                    (
                        sep_conf,
                        appl_a2dp_instance[i].sep_cap.codec_cap.media_type,
                        appl_a2dp_instance[i].sep_cap.codec_cap.codec_type,
                        appl_a2dp_instance[i].sep_cap.codec_cap.codec_ie_len,
                        codec_ie_conf
                    )
                    ...
                    {
                        CONSOLE_OUT("OK. Wait for A2DP_RECONFIGURE_CNF\n");

                        appl_a2dp_save_codec_ie_configuration
                        (
                            appl_a2dp_instance[i].role,
                            appl_a2dp_instance[i].sep_cap.codec_cap.codec_type,
                            codec_ie_conf,
                            appl_a2dp_instance[i].sep_cap.codec_cap.codec_ie_len
                        );
    ...
    case A2DP_START_CNF:
        ...
        if (API_SUCCESS == event_result)
        {
            for (i = 0U; i < A2DP_MAX_CODECS; i++)
            {
                /* start pl has to be done if it is not already started for that role */
            ...
            if (A2DP_MAX_CODECS == i)
            {
                ...
                if (AVDTP_CODEC_AUDIO_SBC == appl_a2dp_instance[codec_instance].codec_type)
                {
                    ...
                    if ((BT_FALSE == appl_a2dp_sbc_encoder_state) &&
                        (AVDTP_SEP_SOURCE == role))
                    {
                        printf("SBC Encoder is not Initialized Succesfully\n");
                        break;
                    }
#endif //A2DP_BRIDGING_APP
#ifdef AVDTP_HAVE_CONTENT_PROTECTION
            ...
            /* print */
            printf ("Max. Media Payload Length Allowed: %d\n", appl_a2dp_instance[codec_instance].max_mpl);
            ...
            /* Update the playback(SINK role) instance if not already used up */
            if ((A2DP_MAX_CODECS == appl_a2dp_playback_instance) &&
                (AVDTP_SEP_SINK == appl_a2dp_instance[codec_instance].role))
            {
                printf ("Update playback instance - %d\n", codec_instance);
...
#endif /* A2DP_SINK */
#endif //#ifdef A2DP_BRIDGING_APP
#ifdef A2DP_BRIDGING_APP
                for (i = 0U; i < A2DP_MAX_CODECS; i++)
                {
                    /*
                    * Find if there are any connections for the bridge application, send Start ind
                    * to that as well
                    */
                    if ((i != codec_instance) &&
                        (appl_a2dp_instance[i].role != appl_a2dp_instance[codec_instance].role) &&
                    ...
                    {
                        /* A2DP Start */
                        CONSOLE_OUT("A2DP Start ... "); fflush(stdout);
                        retval = BT_a2dp_start
                        (
                        ...
                        else
                        {
                            ...
                            CONSOLE_OUT("OK. Wait for A2DP_START_CNF\n");

                            /* Unsniff the link if required */
                            (BT_IGNORE_RETURN_VALUE)BT_hci_get_acl_connection_handle(appl_a2dp_instance[i].bd_addr, &acl_handle);
                            (BT_IGNORE_RETURN_VALUE)BT_hci_exit_sniff_mode(acl_handle);
                        }
                        break;
                    }
    ...
    case A2DP_SUSPEND_CNF:
        ...
        if (API_SUCCESS == event_result)
        {
            ...
            {
                /**
                 * If there are any other SEPs whose role is same as the suspended SEP
                 * and is in the streaming state, then Skip A2DP pl stop
                 */
                if ((APPL_A2DP_STREAM_TRUE == appl_a2dp_instance[i].stream_state) &&
                    (appl_a2dp_instance[i].role == appl_a2dp_instance[codec_instance].role))
                {
...
/* JPL and players are not needed for Bridging app */
#ifndef A2DP_BRIDGING_APP
            if (A2DP_MAX_CODECS == i)
            {
                /**
            ...
            {
                printf ("Reset playback instance\n");
                appl_a2dp_playback_instance = A2DP_MAX_CODECS;
            }
        }
...
/* JPL and players are not needed for Bridging app */
...
#else //A2DP_BRIDGING_APP
            for (i = 0U; i < A2DP_MAX_CODECS; i++)
			{
				/*
				* Find if there are any connections for the bridge application, send Suspend ind
				* to that as well
				*/
				if ((i != codec_instance) &&
					(appl_a2dp_instance[i].role != appl_a2dp_instance[codec_instance].role) &&
					(appl_a2dp_instance[i].stream_state == APPL_A2DP_STREAM_TRUE))
				{
					/* A2DP Suspend */
					CONSOLE_OUT("A2DP Suspend ... "); fflush(stdout);
					retval = BT_a2dp_suspend
					(
						appl_a2dp_instance[i].a2dp_instance
					);
					...
					{
						CONSOLE_OUT("OK. Wait for A2DP_SUSPEND_CNF\n");
					}
					break;
				}
			}
    ...
    case A2DP_RECONFIGURE_CNF:
        ...
        if (API_SUCCESS == event_result)
        {
            ...
            (
                appl_a2dp_instance[codec_instance].codec_type,
                appl_a2dp_instance[codec_instance].codec_ie_conf,
                AVDTP_MAX_CODEC_IE_LEN
            );
        }
...
/* JPL and players are not needed for Bridging app */
            ...
            for (i = 0U; i < A2DP_MAX_CODECS; i++)
            {
                /* If there are any other SEPs whose role is same as the disconnected SEP
                 * and is in the streaming state, then Skip A2DP pl stop
                 */
                if ((APPL_A2DP_STREAM_TRUE == appl_a2dp_instance[i].stream_state) &&
                    (appl_a2dp_instance[i].role == appl_a2dp_instance[codec_instance].role))
            ...
            /* Call shutdown, when all the instances are in NOT in stream state */
            for (i = 0U; i < A2DP_MAX_CODECS; i++)
            {
                if (APPL_A2DP_STREAM_TRUE == appl_a2dp_instance[i].stream_state)
                {
            ...
            /**
             * Free the playback instance
             * current codec_instance is correspond to playback instance.
             */
            if (codec_instance == appl_a2dp_playback_instance)
            {
            	printf("Reset playback instance\n");
    ...
    case A2DP_MEDIA_FRAME_IND:
#if 0
        printf("\n");
        printf("Received A2DP_MEDIA_FRAME_IND\n");
        printf("\tCodec Instance = %d\n", codec_instance);
        printf("\tEvent Result   = 0x%04X\n", event_result);
        printf ("Media Packet Length : %d\n", event_datalen);
        ...
        {   
            ...
            {
                if ((APPL_A2DP_STREAM_TRUE == appl_a2dp_instance[i].stream_state) &&
                    (AVDTP_SEP_SOURCE == appl_a2dp_instance[i].role))
                {
                    while (num_sbc_frames > 0)
                    {
                        ...
                        if (API_SUCCESS != retval)
                        {
                            CONSOLE_OUT("A2DP media write failed - 0x%04X\n", retval);
                        }
#ifndef A2DP_SUPPORT_MULTIPLE_MEDIA_FRAME_WRITE
        ...
        if (AVDTP_CODEC_AUDIO_SBC == appl_a2dp_instance[codec_instance].codec_type)
        {
            ...
            if (API_SUCCESS != retval)
            {
                printf("JPL Add Frames Failed - 0x%04X\n", retval);
            }
#endif /* INCLUDE_JPL */
        }
#endif //A2DP_BRIDGING_APP
    ...
    case A2DP_DELAY_REPORT_IND:
        printf("\n");
        printf("Received A2DP_DELAY_REPORT_IND\n");
        printf("\tCodec Instance = %d\n", codec_instance);
        printf("\tEvent Result   = 0x%04X\n", event_result);
        ...
        printf ("\tA2DP Delay Report Value: %d\n", ((INT16)delay_report_data));
        break;

  case A2DP_DELAY_REPORT_CNF:
        printf("\n");
        printf("Received A2DP_DELAY_REPORT_CNF\n");
        printf("\tCodec Instance = %d\n", codec_instance);
        printf("\tEvent Result   = 0x%04X\n", event_result);
        break;
#endif /* A2DP_1_3 */
...
void appl_a2dp_avdtp_connect (void)
{
    ...
    else
    {
        ...
        /* AVDTP Connect */
        printf("AVDTP Connect ... "); fflush (stdout);
...
void appl_a2dp_get_media_channel_params (void)
{
    ...
    else
    {
        ...
        {
            printf("A2DP Media Channel Params ... "); fflush (stdout);
            ...
            {
                printf("Media Channel params:\n");
                printf("Media Channel ID: 0x%04x\n", appl_a2dp_media_ch_params.lcid);
                printf("Configured MTU: 0x%04x \n",  appl_a2dp_media_ch_params.mtu);
            }
        }
    }
...
void appl_a2dp_avdtp_discover (void)
{
    ...
    else
    {
        ...
        /* AVDTP Discover */
        printf("AVDTP Discover ... "); fflush (stdout);
...
void appl_a2dp_avdtp_get_capabilities (void)
{
    ...
    else
    {
        ...
        printf("AVDTP Get Capabilities ... "); fflush (stdout);
...
void appl_a2dp_avdtp_disconnect (void)
{
    ...
    else
    {
        ...
        /* AVDTP Disconnect */
        printf("AVDTP Disconnect ... "); fflush (stdout);
        ...
        {
            printf("OK. Wait for AVDTP_DISCONNECT_CNF\n");
        }
    }
...
void appl_a2dp_avdtp_get_all_capabilities (void)
{
    ...
    else
    {
        ...
        printf("AVDTP Get All Capabilities ... "); fflush (stdout);
        ...
        {
            printf("OK. Wait for AVDTP_GET_ALL_CAPABILITIES_CNF\n");
        }
    }
...
void appl_a2dp_set_initial_delay_report (UCHAR inst)
{
    ...
    {
        printf ("Enter Delay Report Value(in decimal frmt[multiple of 0.1 milliseconds])\n");
        scanf ("%d", &delay_val);
        ...
        {
            printf ("Updated Initial Delay Report\n");
        }
    }
    else
    {
...
void appl_a2dp_send_delay_report (UCHAR inst)
{
    ...
    if (AVDTP_SEP_SINK == appl_a2dp_instance[inst].sep_cap.sep_type)
    {
        ...
        printf("AVDTP Send Delay Report... "); fflush (stdout);
        ...
        {
            printf("OK. Wait for AVDTP_DELAY_REPORT_CNF\n");
        }
    }
    else
    {
           ...
           )
{
    ...
    case AVDTP_GET_ALL_CAPABILITIES_CNF:
        ...
        {
            printf ("\tDelay Reporting Service: YES \n");
...
void appl_a2dp_get_instance (UCHAR * indx)
{
    ...
    BT_LOOP_FOREVER()
    {
        printf("Choose A2DP Instance = "); fflush(stdout);
        scanf("%d", &choice);
...
API_RESULT appl_a2dp_get_acl_connection (UCHAR * indx)
{
    ...
    if (API_SUCCESS != retval)
    {
         printf("*** HCI Get Connection Details FAILED !! "\
         "Error Code = 0x%04X\n", retval);
    ...
    else
    {
        if (0U == num_list)
        {
            printf("*** ACL Connection to a Remote Device NOT Available\n");
            retval = API_FAILURE; /* return API_FAILURE; */
        }
        else
        {
      ...
      )
{
...
#else
    retval = APPL_A2DP_AVOID_FILE_DEFAULT_CONFIG;
#endif
    if (retval < 0)
    {
#if 0
        printf("Read Configuration: Sampling Frequency NOT found\n");
        return retval;
#else /* 0 */
        retval = 0;
    ...
    default:
        printf("Read Configuration: Invalid Sampling Frequency\n");
        retval = -1; /* return -1; */
    }
    ...
    if (0 == retval)
    {
        ...
        {
    #if 0
            printf("Read Configuration: SBC Channel Mode NOT found\n");
            return retval;
    #else /* 0 */
            retval = 0;
        ...
        {
            printf("Read Configuration: Channel Mode -> %s\n", config_buf);
        }
        ...
        {
            printf("Read Configuration: Invalid Channel Mode\n");
            retval = -1; /* return -1; */
        }
    }
    ...
    if (0 == retval)
    {
        ...
        {
    #if 0
            printf("Read Configuration: SBC Block Length NOT found\n");
            return retval;
    #else /* 0 */
            retval = 0;
    #ifdef APPL_A2DP_DEFAULT_CONFIG
        ...
        {
            (BT_IGNORE_RETURN_VALUE) sscanf(config_buf, "%d", &option);
            printf("Read Configuration: SBC Block Length -> %d\n", option);
        }
        ...
        default:
            printf("Read Configuration: Invalid SBC Block Length\n");
            retval = -1; /* return -1; */
        }
    }
    ...
    if (0 == retval)
    {
        ...
        {
    #if 0
            printf("Read Configuration: SBC Subband NOT found\n");
            return retval;
    #else /* 0 */
            retval = 0;
    #ifdef APPL_A2DP_DEFAULT_CONFIG
        ...
        {
            (BT_IGNORE_RETURN_VALUE) sscanf(config_buf, "%d", &option);
            printf("Read Configuration: SBC Subband -> %d\n", option);
        }
        ...
        default:
            printf("Read Configuration: Invalid SBC Subband\n");
            retval = -1; /* return -1; */
        }
    }
    ...
    if (0 == retval)
    {
        ...
        {
    #if 0
            printf("Read Configuration: SBC Allocation Method NOT found\n");
            return retval;
    #else /* 0 */
            retval = 0;
    #ifdef APPL_A2DP_DEFAULT_CONFIG
        ...
        {
            printf("Read Configuration: Allocation Method -> %s\n", config_buf);
        }
        ...
        {
            printf("Read Configuration: Invalid Allocation Method\n");
            retval = -1; /* return -1; */
        }
    }
    ...
    if (0 == retval)
    {
        ...
        {
    #if 0
            printf("Read Configuration: SBC Bitpool NOT found\n");
            return retval;
    #else /* 0 */
        ...
        {
            printf("Read Configuration: Invalid SBC Bitpool\n");
            retval = -1; /* return -1; */
        }
    }
      ...
      )
{
    ...
    printf ("===== Read Configuration of MPEG-1,2 Codec =====\n");
    ...
    if (retval >= 0)
    {
        ...
        if (retval < 0)
        {
            printf("\t    Media Payload Format NOT found\n");
            /* return retval; */
        }
        else
        {
    ...
    if (retval >= 0)
    {
        ...
        if (retval < 0)
        {
            printf("\t    Sampling Frequeny NOT found\n");
            /* return retval; */
        }
        else
        {
            sscanf(config_buf, "%d", &option);
            printf("\t    Sampling Frequeny -> %d\n", option);
    ...
    if (retval >= 0)
    {
        ...
        if (retval < 0)
        {
            printf("\t    Variable Bit Rate NOT found\n");
            /* return retval; */
        }
        else
        {
            printf("\t    Variable Bit Rate -> %s\n", config_buf);
    ...
    if (retval >= 0)
    {
        ...
        if (retval < 0)
        {
            printf("\t    Bit Rate index NOT found\n");
            /* return retval; */
        }
        else
        {
            sscanf(config_buf, "%d", &option);
            printf("\t    Bit Rate index -> %d\n", option);
      ...
      )
{
    ...
    printf ("===== Read Configuration of MPEG-2,4 Codec =====\n");
       ...
       )
{
    ...
    {
        /* printf("<%d> ", r_datalen); */
        (BT_IGNORE_RETURN_VALUE)app_standalone_a2dp_src_enqueue(r_data, r_datalen);
    }
...
DECL_STATIC BT_THREAD_RETURN_TYPE a2dp_src_write_task (BT_THREAD_ARGS args)
{
    ...
    BT_LOOP_FOREVER()
    {
        ...
        while (buf_index < bytes_to_send)
        {
            ...
            (BT_IGNORE_RETURN_VALUE) a2dp_standalone_encode_n_send
            (
...
API_RESULT appl_a2dp_media_write(void)
{
    ...
    for (i = 0U; i < A2DP_MAX_CODECS; i++)
    {
        ...
        if (APPL_A2DP_STREAM_TRUE == appl_a2dp_instance[i].stream_state)
        {
            ...
            printf ("\n==========\n");
            printf ("0x%02X, 0x%02X, 0x%02X", appl_sbc_data[0U], appl_sbc_data[1U], appl_sbc_data[2U]);
            printf ("\n===========\n");
      ...
      )
{
    ...
    {
        printf ("NULL parameter in Read Codec Configuration\n");
        /* return ret; */
    }
    else
    {
...
/* jpl configuration and src prep is not needed for a2dp bridging app */
        ...
        if (AVDTP_CODEC_AUDIO_SBC == codec_type)
        {
            ...
            {
                appl_standalone_a2dp_src_prepare();
            }
#endif /* A2DP_SOURCE */
        }
#endif //A2DP_BRIDGING_APP
      ...
      )
{
    ...
    else
    {
        if (AVDTP_CODEC_AUDIO_SBC == codec_type)
        {
            ...
            if (A2DP_SBC_CODEC_CAP_IE_LEN == codec_ie_len)
            {
                printf("\tSBC Codec Capability IE = 0x%02X 0x%02X 0x%02X 0x%02X\n",
                codec_ie[0U], codec_ie[1U], codec_ie[2U], codec_ie[3U]);
            ...
            {
                printf("*** A2DP_CAPABILITY_CNF: No Event Data/Data Length\n");
            }
        }
#ifdef A2DP_MPEG_1_2
        else if (AVDTP_CODEC_AUDIO_MPEG_1_2 == codec_type)
        {
            ...
            if (A2DP_MPEG_1_2_CODEC_CAP_IE_LEN == codec_ie_len)
            {
                printf("\tMPEG-1,2 Codec Capability IE = 0x%02X 0x%02X 0x%02X 0x%02X\n",
                codec_ie[0U], codec_ie[1U], codec_ie[2U], codec_ie[3U]);
...
#ifdef A2DP_BRIDGING_APP
        appl_a2dp_instance[inst].a2dp_connection_state = APPL_A2DP_DISCONNECTED;
#endif //A2DP_BRIDGING_APP
    }
...
void appl_avdtp_set_sig_msg_reject_rsp_params(void)
{
    ...
    printf ("Enter AVDTP Signalling ID(in Hex): ");
    scanf ("%x", &read_val);
    sig_id = (UCHAR)read_val;

    printf ("Enter Error Code(in Hex): ");
    scanf ("%x", &read_val);
    ...
    if(API_SUCCESS != retval)
    {
        printf ("BT_avdtp_set_sig_msg_reject_rsp_params Failed:(0x%04x)\n", retval);
    }
