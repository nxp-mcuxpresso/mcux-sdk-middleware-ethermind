...
APPL_EVENT_HANDLER_CB appl_event_hndlr_cb = NULL;
           ...
           )
{
    ...
    else
    {
        ...
        case ATT_XCHNG_MTU_REQ:
            ...
            else if (API_SUCCESS == retval)
            {
                ...
                {
                    BT_UNPACK_LE_2_BYTE(&mtu, eventdata);
                    APPL_TRC (
        ...
        case ATT_FIND_BY_TYPE_VAL_REQ:
            ...
            if (API_SUCCESS == event_result)
            {
                ...
                uuid.val = &eventdata[offset];

                offset += 2U;
                value.len = event_datalen - offset;
                value.val = &eventdata[offset];
        ...
        case ATT_WRITE_CMD:
            if (API_SUCCESS == event_result)
            {
                ...
                appl_dump_bytes (&eventdata[2U], event_datalen - 2U);
            }
            break;
#ifdef ATT_QUEUED_WRITE_SUPPORT
        ...
        case ATT_EXECUTE_WRITE_REQ:
            if (NULL != eventdata)
            {
                ...
                if (0x02U > eventdata[0U])
                {
                    (BT_IGNORE_RETURN_VALUE) appl_execute_write_handler[eventdata[0U]](handle);
                }
            }
            else
            {
        ...
        case ATT_HANDLE_VALUE_NTF_TX_COMPLETE:
            appl_parse_ntf_tx_complete (handle, eventdata, event_datalen);
            break;
        ...
        case ATT_HANDLE_VALUE_MULTIPLE_NTF_TX_COMPLETE:
            ...
            appl_dump_bytes(eventdata, event_datalen);
            break;
        ...
        case ATT_SIGNED_WRITE_CMD:
...
#ifdef SMP_DATA_SIGNING
            {
                ...
                BT_mem_copy (&s_data[1U], eventdata, event_datalen);
    ...
    case ATT_READ_MULTIPLE_VARIABLE_LENGTH_REQ:
    {
        ...
        appl_handle_read_multiple_var_len_request (handle,&list);
    }
    break;
#endif /* ATT_READ_MULTIPLE_VARIABLE_LENGTH_SUPPORT */
    ...
    case ATT_UNKNOWN_PDU_IND:
        ...
        if (NULL == eventdata)
        {
            ...
            if (BT_FALSE != APPL_CHECK_IF_ATT_REQUEST(eventdata[0U]))
            {
                (BT_IGNORE_RETURN_VALUE) appl_handle_unsupported_op_code (handle,eventdata[0U]);
            }
            else
            {
                APPL_ERR (
                 ...
                 */
                err_param.error_code = ATT_REQUEST_NOT_SUPPORTED;
                err_param.op_code = eventdata[0U];
                err_param.handle = ATT_INVALID_ATTR_HANDLE_VAL;
...
API_RESULT appl_gatt_send_multi_handle_val_ntf_request(DEVICE_HANDLE dq_handle)
{
    ...
    retval = BT_att_send_hndl_val_multiple_ntf(&APPL_GET_ATT_INSTANCE(appl_handle), &param);
