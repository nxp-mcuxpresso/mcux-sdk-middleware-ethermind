...
void appl_service_init (void)
{
#ifdef GATT_DB_DYNAMIC
#ifdef GATT_DB_HAVE_DB_SIGNATURE
    GATT_DB_SIGNATURE appl_gatt_db_sign;
#endif /* GATT_DB_HAVE_DB_SIGNATURE */
    ...
    (BT_IGNORE_RETURN_VALUE)appl_add_gaps();
    (BT_IGNORE_RETURN_VALUE)appl_add_gatts();
    (BT_IGNORE_RETURN_VALUE)appl_add_bas();
    (BT_IGNORE_RETURN_VALUE)appl_add_dis();
...
#ifdef GATT_DB_DYNAMIC
#ifdef GATT_DB_HAVE_DB_SIGNATURE
    /* Set the GATT DB Signature */
    appl_gatt_db_sign = APPL_GATT_DB_SIGN;
    retval = BT_gatt_db_dyn_set_db_signature(appl_gatt_db_sign);
#endif /* GATT_DB_HAVE_DB_SIGNATURE */
    ...
    APPL_TRC(
    "[APPL]: GATT Database Registration Status: 0x%04X\n", retval);
    ...
    gatt_db_init_pl(gatt_db_gatt_char_handler);
}
...
#ifdef PXR
    appl_pxr_connect(&APPL_GET_DEVICE_HANDLE((*handle)));
#endif /* PXR */

#ifdef BT_HRS
    appl_hrs_connect(&APPL_GET_DEVICE_HANDLE((*handle)));
#endif /* BT_HRS */

#ifdef VSE
    appl_vse_connect(&APPL_GET_DEVICE_HANDLE((*handle)));
#endif /* VSE */
}
...
#ifdef PXR
    appl_pxr_reinit(&APPL_GET_DEVICE_HANDLE(*(handle)), (reason));
#endif /* PXR */
     ...
     )
{
    ...
    BT_IGNORE_UNUSED_PARAM(scan_type);
    BT_IGNORE_UNUSED_PARAM(l_addr_type);
    BT_IGNORE_UNUSED_PARAM(filter_policy);
           ...
           )
{
    ...
    BT_IGNORE_UNUSED_PARAM(l_addr_type);
    BT_IGNORE_UNUSED_PARAM(p_addr_type);
    BT_IGNORE_UNUSED_PARAM(p_addr);
    BT_IGNORE_UNUSED_PARAM(filter_policy);
    BT_IGNORE_UNUSED_PARAM(use_config);
...
#ifdef BT_HRS
    appl_hrs_handle_ind_complete(&appl_hndl, evt_result);
#endif /* BT_HRS */
...
#ifdef PXR
    appl_pxr_handle_ntf_complete(&appl_hndl, data, datalen);
#endif /* PXR */

#ifdef BT_HRS
    appl_hrs_handle_ntf_complete(&appl_hndl, data, datalen);
#endif /* BT_HRS */
...
#ifdef PXR
    appl_pxr_handle_mtu_update_complete(&appl_hndl, mtu);
#endif /* PXR */

#ifdef BT_HRS
    appl_hrs_handle_mtu_update_complete(&appl_hndl, mtu);
#endif /* BT_HRS */
