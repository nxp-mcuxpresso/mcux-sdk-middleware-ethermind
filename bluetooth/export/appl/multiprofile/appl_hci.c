...
extern void appl_sdp_open (void);
extern UCHAR appl_acl_initiated;

/* ----------------------------------------- Static Global Variables */
#ifdef MULTI_PROFILE_FSM_SUPPORT
	UCHAR sco_bd_addr[BT_BD_ADDR_SIZE];
#endif /* MULTI_PROFILE_FSM_SUPPORT */

UINT16 appl_peer_acl_handle;
UCHAR appl_peer_bd_addr[BT_BD_ADDR_SIZE];
/*
 * Array of discovered devices
 * Duplicate entry will not be maintained in this array
 */
static INQUIRY_RESULT appl_inquiry_res[INQUIRY_NUM_RESPONSES];
static INQUIRY_RESULT cur_inq_res;
static void appl_extract_bdaddr_inquiry_result(UCHAR* event_data);
static UINT8 appl_get_free_inq_res_index();
static UINT8 appl_get_discovered_device_index(INQUIRY_RESULT cur_inq_res);
...
#ifdef HFP_BRIDGING
#define MAX_SCO_CONN 2
#define HEX_STREAM_LEN 49U
#define CHAR_STREAM_LEN 17U
#define ASCII_SPACE 0x20U
#define ASCII_TILDE 0x7EU
DECL_STATIC UINT16 sco_connection_handle[MAX_SCO_CONN] = {HCI_INVALID_CONNECTION_HANDLE, HCI_INVALID_CONNECTION_HANDLE};
#else
           ...
           )
{
    ...
    case HCI_INQUIRY_RESULT_EVENT:
        ...
        for (count = 0U; count < num_responses; count ++)
        {
			...
			printf("\tBD_ADDR: ");
			appl_extract_bdaddr_inquiry_result(event_data);
			event_data += 6U;

			/* Page Scan Repetition Mode */
			hci_unpack_1_byte_param(&value_1, event_data);
			printf("\tPage Scan Repetition Mode: 0x%02X\n", value_1);
			cur_inq_res.page_scan_repetition_mode = value_1;
			event_data += 1U;
			...
			cur_inq_res.class_of_device = value_3;
			event_data += 3U;

			/* Clock Offset */
			hci_unpack_2_byte_param(&value_2, event_data);
			printf("\tClock Offset: 0x%04X\n", value_2);
			cur_inq_res.clock_offset = value_2;
			event_data += 2U;

			cur_inq_res.valid = 1U;
			printf(
			"-------------------------------------------------------------------\n");
			/* Update the discovered devices list */
			UINT8 dev_idx = 0xFFU;
			dev_idx = appl_get_discovered_device_index(cur_inq_res);
			if(0xFF == dev_idx)
			{
				/* Device Discovered for the first time */
				UINT8 index;
				index = appl_get_free_inq_res_index();
				if(index < INQUIRY_NUM_RESPONSES)
				{
					BT_mem_copy(&appl_inquiry_res[index], &cur_inq_res,sizeof(INQUIRY_RESULT));
				}
			}
			else
			{
				/* Update the discovered device entry */
				appl_inquiry_res[dev_idx].clock_offset = cur_inq_res.clock_offset;
				appl_inquiry_res[dev_idx].rssi = cur_inq_res.rssi;
			}
			BT_mem_set(&cur_inq_res, 0x0U, sizeof(INQUIRY_RESULT));
        }
        break;
    ...
    case HCI_EXTENDED_INQUIRY_RESULT_EVENT:
        ...
        for (count = 0U; count < num_responses; count ++)
        {
			...
			{
				/* Update the discovered devices entry */
				appl_inquiry_res[dev_idx].clock_offset = cur_inq_res.clock_offset;
				appl_inquiry_res[dev_idx].rssi = cur_inq_res.rssi;
				/* Sometimes EIR with length 0 is received. In that case, update(if needed)
				 * the name only when the correct EIR is received for the same device */
				if((0 != BT_mem_cmp(appl_inquiry_res[dev_idx].local_name, cur_inq_res.local_name, strlen((char *)cur_inq_res.local_name)))
						&& (0 != (strlen((char *)cur_inq_res.local_name))))
				{
					BT_mem_copy(appl_inquiry_res[dev_idx].local_name, cur_inq_res.local_name, strlen((char *)cur_inq_res.local_name));
				}
			}
			BT_mem_set(&cur_inq_res, 0x0U, sizeof(INQUIRY_RESULT));
        }
    ...
    case HCI_CONNECTION_COMPLETE_EVENT:
...
#ifdef HFP_BRIDGING
            int i = 0;
            for(i = 0; i < MAX_SCO_CONN; i++)
            {
            	if(HCI_INVALID_CONNECTION_HANDLE == sco_connection_handle[i])
            	{
            		sco_connection_handle[i] = connection_handle;
            		break;
            	}
            }
            /* if there was no 2 sco connections earlier */
            if(i < MAX_SCO_CONN)
            {
            	i = 0;
            	/* check if there are 2 sco connections now */
				for(i = 0; i < MAX_SCO_CONN; i++)
				{
					if(HCI_INVALID_CONNECTION_HANDLE == sco_connection_handle[i])
					{
				...
				if(i == MAX_SCO_CONN)
				{
					sco_bridge_audio_start_pl(sco_connection_handle[0], sco_connection_handle[1]);
				}
            }
#else
        ...
        if (BT_TRUE == appl_acl_initiated)
        {
            appl_acl_initiated = BT_FALSE;

            if ((0x00 == status) &&
                (HCI_ACL_LINK == link_type) &&
                (BT_TRUE == BT_COMPARE_ADDR(bd_addr, appl_peer_bd_addr)))
            {
                /* Save the connection handle */
                appl_peer_acl_handle = connection_handle;
                printf ("calling appl_sdp_open\n");
                appl_sdp_open ();
            }
        }
        break;
    ...
    case HCI_CONNECTION_REQUEST_EVENT:
	{
        ...
        case HCI_ESCO_LINK:
            ...
            profile_info.data               = (UCHAR*)sco_bd_addr;
...
#ifdef HFP_BRIDGING
        uint8_t sco_disconnection = 0;
        for(int i = 0; i < MAX_SCO_CONN; i++)
        {
        	if((sco_connection_handle[i] == connection_handle) /* && ((link_type == HCI_SCO_LINK) || (link_type == HCI_ESCO_LINK))*/)
        	{
				/* Reset SCO connection handle */
				sco_connection_handle[i] = HCI_INVALID_CONNECTION_HANDLE;
				sco_disconnection = 1U;
				break;
        	}
        }

        if (sco_disconnection == 1U)
        {
#ifdef MULTI_PROFILE_FSM_SUPPORT
			profile_info.event_type         = ev_pro_hfp_sco_disconnect;
			profile_info.event_info         = PROFILE_FSM_LOCAL_EVENT;
			profile_info.profile_fsm_handle = get_multiprofile_handle();
			profile_info.data               = (UCHAR*)sco_bd_addr;
			profile_info.datalen            = BT_BD_ADDR_SIZE;
			...
			printf ("called sco_audio_stop_pl, Voice stopped\n");
        }
#else
    ...
    case HCI_SYNCHRONOUS_CONNECTION_COMPLETE_EVENT:
             ...
             (HCI_ESCO_LINK == link_type)))
        {
            ...
            for(i = 0; i < MAX_SCO_CONN; i++)
            {
            	...
            	{
            		sco_connection_handle[i] = connection_handle;
            		printf("sco handle added at index %d, handle = %x\n", i, sco_connection_handle[i]);
            		break;
            	}
            }
            /* if there was no 2 sco connections earlier */
            ...
            {
            	i = 0;
            	printf("there was no 2 sco connections earlier\n");
            	/* check if there are 2 sco connections now */
				for(i = 0; i < MAX_SCO_CONN; i++)
				{
					if(HCI_INVALID_CONNECTION_HANDLE == sco_connection_handle[i])
					{
						printf("there is still a free index %d\n", i);
						break;
					}
				}
				if(i == MAX_SCO_CONN)
				{
					printf("hfp-ag hfp-unit sco created, starts sco-bridge\n");
#ifdef MULTI_PROFILE_FSM_SUPPORT
					for (i = 0; i < BT_BD_ADDR_SIZE; i++ )
					{
						sco_bd_addr[i] = event_data[i];
...
#endif /* HCI_NO_ESCO_AUTO_ACCEPT */
#ifdef HFP_BRIDGING
	appl_enh_sco_params.packet_type = 0x0380U;
	appl_enh_sco_params.max_latency = 0x000DU;
	appl_enh_sco_params.retransmission_effort = 0x02U;
#endif //HFP_BRIDGING
}
...
#ifndef HCI_NO_ESCO_AUTO_ACCEPT
#ifdef BT_HCI_1_2
#if 0 /* Defined in appl_common.c Re-visit*/
    ...
    else
    {
        ...
        while(consumed_length < eir_datalen)
        {
            ...
            for (index = 0U; index < (length - 1U); index ++)
            {
                ...
                {
                    printf("%c", *eir_data);
                    if(index < (INQUIRY_LOCAL_NAME_LEN-1))
                    {
                    	cur_inq_res.local_name[index] = *eir_data;
                    }
                }
                else
                {
            ...
            if (0U != name)
            {
            	(index < INQUIRY_LOCAL_NAME_LEN) ? (cur_inq_res.local_name[index] = '\0')
            			: (cur_inq_res.local_name[INQUIRY_LOCAL_NAME_LEN-1] = '\0');
            }
            printf("\n");
...
/* APIs related to Inquiry and Discovered Device List */
void appl_reset_hci_inquiry_result()
{
	BT_mem_set(appl_inquiry_res, 0, INQUIRY_NUM_RESPONSES * sizeof(INQUIRY_RESULT));
	BT_mem_set(&cur_inq_res, 0, sizeof(INQUIRY_RESULT));
}
...
void appl_print_discovered_devices()
{
	...
	for(UINT8 i = 0; i < INQUIRY_NUM_RESPONSES; i++)
	{
		if(inq_result[i].valid)
		{
			printf("\n%d. bdaddr = %02X %02X %02X %02X %02X %02X \n", i, inq_result[i].bd_addr[0], \
					inq_result[i].bd_addr[1], inq_result[i].bd_addr[2], inq_result[i].bd_addr[3], \
					inq_result[i].bd_addr[4], inq_result[i].bd_addr[5]);
			printf("   page_scan_repetition_mode = 0x%X\n", inq_result[i].page_scan_repetition_mode);
			printf("   class_of_device = 0x%X\n", inq_result[i].class_of_device);
			printf("   clock offset = 0x%X\n", inq_result[i].clock_offset);
			printf("   rssi = 0x%X\n", inq_result[i].rssi);
...
{
	UINT8 index = 0;
	for(index = 0; index < INQUIRY_NUM_RESPONSES; index++)
	{
		if(appl_inquiry_res[index].valid == 0U)
		{
			break;
		}
	}
...
static UINT8 appl_get_discovered_device_index(INQUIRY_RESULT cur_inq_res)
{
	...
	UINT8 idx = 0;
	UINT8 device_already_discovered = BT_FALSE;

	inq_res = appl_get_hci_inquiry_result();

	for(idx = 0; idx < INQUIRY_NUM_RESPONSES; idx++)
	{
		/* Since devices are inserted sequentially,
		 * we can break when one invalid entry is encountered
		 */
		if(0 == inq_res[idx].valid)
		{
			break;
		}
		else if(0 == BT_mem_cmp(inq_res[idx].bd_addr, cur_inq_res.bd_addr, BT_BD_ADDR_SIZE))
		{
			device_already_discovered = BT_TRUE;
			break;
		}
	}
...
static void appl_extract_bdaddr_inquiry_result(UCHAR* event_data)
{
	char hex_stream[HEX_STREAM_LEN];
	char char_stream[CHAR_STREAM_LEN];
	UINT32 i;
	UINT16 offset, count;
	UCHAR c;
	...
	count = 0U;
	offset = 0U;
	for(i = 0U; i < BT_BD_ADDR_SIZE; i++)
	{
		c =  event_data[i];
		cur_inq_res.bd_addr[i] = c;
		...
		if(16U == count)
		{
			...
			BT_mem_set(hex_stream, 0, HEX_STREAM_LEN);
			BT_mem_set(char_stream, 0, CHAR_STREAM_LEN);
		}
	}
	if(offset != 0U)
	{
...
/* APIs related to Inquiry and Discovered Device List - ENDS*/
